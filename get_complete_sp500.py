    ...
    for file_path in possible_files:
        if Path(file_path).exists():
            ...
            try:
                ...
                ticker_cols = ['Symbol', 'Ticker', 'ticker', 'symbol', 'TICKER', 'SYMBOL']
                for col in ticker_cols:
                    if col in df.columns:
                        tickers = df[col].dropna().astype(str).str.strip().str.upper()
                        logger.info(f"Found {len(tickers)} S&P 500 tickers in {file_path}")
                        return set(tickers)
...
def get_sp500_alternative_sources():
        ...
        if response.status_code == 200:
            ...
            if tables:
                ...
                if 'Symbol' in sp500_df.columns:
                    ...
                    logger.info(f"Successfully got {len(clean_tickers)} S&P 500 tickers from Wikipedia")
    ...
    sp500_comprehensive = {
        ...
        'INTU', 'NOW', 'PANW', 'AMAT', 'ADI', 'LRCX', 'KLAC', 'CDNS', 'SNPS', 'MCHP',
        'FTNT', 'ANSS', 'CTSH', 'FISV', 'FIS', 'PAYX', 'ADP', 'MSCI', 'VRSK', 'VRSN',
        
        # Healthcare
        'UNH', 'JNJ', 'PFE', 'ABBV', 'LLY', 'TMO', 'ABT', 'DHR', 'MRK', 'BMY',
        'AMGN', 'GILD', 'VRTX', 'REGN', 'ZTS', 'SYK', 'BDX', 'EW', 'ISRG', 'BSX',
        'MDT', 'IQV', 'DXCM', 'BIIB', 'IDXX', 'WST', 'MTD', 'ALGN', 'MRNA', 'TECH',
        'A', 'BAX', 'BIO', 'HOLX', 'ILMN', 'INCY', 'PKI', 'RMD', 'RVTY', 'STE',
        'TFX', 'VAR', 'VTRS', 'WAT', 'XRAY', 'ZBH',
        
        # Financials
        'BRK-B', 'JPM', 'V', 'MA', 'BAC', 'WFC', 'GS', 'MS', 'C', 'AXP',
        'SCHW', 'BLK', 'SPGI', 'CME', 'ICE', 'MCO', 'MMC', 'AON', 'PNC', 'USB',
        'TFC', 'COF', 'AIG', 'MET', 'PRU', 'ALL', 'TRV', 'AFL', 'PGR', 'CB',
        'AJG', 'BRO', 'HIG', 'CINF', 'L', 'RE', 'WRB', 'ACGL', 'AIZ', 'FNF',
        
        # Consumer Discretionary
        'AMZN', 'HD', 'MCD', 'NKE', 'LOW', 'SBUX', 'TJX', 'BKNG', 'ABNB', 'CMG',
        'ORLY', 'AZO', 'LULU', 'RCL', 'CCL', 'NCLH', 'MAR', 'HLT', 'MGM', 'WYNN',
        'LVS', 'CZR', 'DRI', 'YUM', 'QSR', 'DPZ', 'DPZ', 'BBY', 'EBAY', 'ETSY',
        'W', 'CHWY', 'CVNA', 'KMX', 'AN', 'LAD', 'PAG', 'SIG', 'TPG', 'TSCO',
        
        # Consumer Staples  
        'PG', 'KO', 'PEP', 'WMT', 'COST', 'MDLZ', 'PM', 'MO', 'CL', 'KMB',
        'GIS', 'K', 'HSY', 'SJM', 'CAG', 'CPB', 'HRL', 'MKC', 'CLX', 'CHD',
        'COTY', 'EL', 'KR', 'SYY', 'DLTR', 'DG', 'BJ', 'COST', 'WBA', 'CVS',
        
        # Energy
        'XOM', 'CVX', 'COP', 'EOG', 'SLB', 'PSX', 'VLO', 'MPC', 'OXY', 'BKR',
        'HAL', 'DVN', 'FANG', 'EQT', 'CTRA', 'MRO', 'APA', 'OVV', 'TPG', 'WMB',
        'KMI', 'LNG', 'TRGP', 'EPD', 'ET', 'MPLX', 'PAA', 'WES', 'DT', 'ENLC',
        
        # Industrials
        'CAT', 'RTX', 'HON', 'UNP', 'DE', 'LMT', 'BA', 'GE', 'MMM', 'ITW',
        'EMR', 'ETN', 'PH', 'JCI', 'CARR', 'OTIS', 'GD', 'NOC', 'LHX', 'TDG',
        'AXON', 'HWM', 'IEX', 'FTV', 'AME', 'DOV', 'GNRC', 'PWR', 'HUBB', 'BLDR',
        'SNA', 'PNR', 'ROK', 'FAST', 'PAYX', 'CHRW', 'EXPD', 'JBHT', 'ODFL', 'XPO',
        
        # Materials
        'LIN', 'APD', 'SHW', 'FCX', 'NEM', 'ECL', 'DD', 'DOW', 'PPG', 'VMC',
        'MLM', 'NUE', 'STLD', 'RS', 'RPM', 'AVY', 'BALL', 'AMCR', 'PKG', 'IP',
        'WRK', 'SEE', 'SON', 'ALB', 'FMC', 'LYB', 'CE', 'CF', 'MOS', 'IFF',
        
        # Real Estate
        'PLD', 'AMT', 'CCI', 'EQIX', 'WELL', 'DLR', 'PSA', 'O', 'CBRE', 'IRM',
        'AVB', 'EQR', 'ESS', 'MAA', 'UDR', 'CPT', 'FRT', 'REG', 'BXP', 'VTR',
        'PEAK', 'ARE', 'DOC', 'EXR', 'INVH', 'KIM', 'SPG', 'SLG', 'VNO', 'WY',
        
        # Utilities
        'NEE', 'SO', 'DUK', 'AEP', 'SRE', 'D', 'PEG', 'EXC', 'XEL', 'WEC',
        'ED', 'ETR', 'ES', 'FE', 'EIX', 'PPL', 'AEE', 'DTE', 'NI', 'LNT',
        'CMS', 'CNP', 'ATO', 'NRG', 'VST', 'CEG', 'PCG', 'PNW', 'AWK', 'WTRG',
        
        # Communication Services
        'GOOGL', 'GOOG', 'META', 'NFLX', 'DIS', 'CMCSA', 'VZ', 'T', 'CHTR', 'TMUS',
        'PARA', 'WBD', 'FOXA', 'FOX', 'NWSA', 'NWS', 'MTCH', 'PINS', 'SNAP', 'TWTR',
        'LUMN', 'SIRI', 'DISH', 'CABO', 'LILAK', 'LILA', 'LBRDA', 'LBRDK', 'FWONA', 'FWONK'
    }
...
def main():
    ...
    if sp500_tickers and len(sp500_tickers) > 400:  # Should be close to 500
        ...
        logger.info(f"Saved S&P 500 tickers to {output_dir}/sp500_tickers_retrieved.csv")
        return sp500_tickers
    else:
        logger.error(f"Failed to get complete S&P 500 list. Got {len(sp500_tickers) if sp500_tickers else 0} tickers")
        return None
