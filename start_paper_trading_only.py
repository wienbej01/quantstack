...
def main():
    ...
    l2_collector = QuantstackL2Collector(
        symbols=symbols[:6],  # Top 6 for L2 collection
    ...
    try:
        while True:
            ...
            logger.info(f"🔄 Cycle {cycle_count} - {current_time.strftime('%H:%M:%S ET')}")
            
            # Check if market is open (9:30-16:00 ET)
            market_time = current_time.time()
            if not (market_time >= datetime.strptime("09:30", "%H:%M").time() and 
                   market_time <= datetime.strptime("16:00", "%H:%M").time()):
            ...
            try:
                ...
                logger.info(f"✅ Cycle complete - {cycle_time:.1f}s, {len(trades)} trades")
