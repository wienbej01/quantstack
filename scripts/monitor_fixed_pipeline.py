...
def check_pipeline_status():
    ...
    try:
        result = subprocess.run(
            ["pgrep", "-f", "build_intraday_features_fixed"],
            check=False,
            capture_output=True,
            text=True,
        )
    ...
    if results_exist:
        try:
            ...
            if len(trades) > 0:
                ...
                print(
                    f"Performance: {win_rate:.1%} win rate, ${total_pnl:,.0f} total PnL"
                )
        except:
...
if __name__ == "__main__":
    ...
    if running:
        ...
        print("   python scripts/monitor_fixed_pipeline.py")
