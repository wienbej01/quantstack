...
def tune_parameters():
    ...
    for thresh, atr_mult, hours in product(thresholds, atr_multipliers, hour_filters):
        try:
            result = test_config(train_df, test_df, feature_cols, thresh, atr_mult, hours)
            if result:
                results.append(result)
                logging.info(f"Config: thresh={thresh}, atr={atr_mult}, hours={len(hours)} → "
    ...
    print(top_win[['threshold', 'atr_mult', 'hour_count', 'trades', 'win_rate', 'pnl', 'symbols']].to_string(index=False))
    
    logging.info("\n" + "=" * 80)
    logging.info("TOP CONFIGS BY PNL")
    logging.info("=" * 80)
    top_pnl = results_df.nlargest(5, 'pnl')
    print(top_pnl[['threshold', 'atr_mult', 'hour_count', 'trades', 'win_rate', 'pnl', 'symbols']].to_string(index=False))
    
    # Best overall (balance of win rate and PnL)
    results_df['score'] = results_df['win_rate'] * np.log(results_df['trades'].clip(lower=1)) + results_df['pnl'] / 1000
    ...
    logging.info(f"Performance: {best['trades']} trades, {best['win_rate']:.1f}% win, ${best['pnl']:,.0f}")
...
def test_config(train_df, test_df, feature_cols, threshold, atr_mult, hours):
    ...
    train_df['label_long_new'] = (train_df['forward_return'] > train_df['atr_threshold_new']).astype(int)
    train_df['label_short_new'] = (train_df['forward_return'] < -train_df['atr_threshold_new']).astype(int)
    
    test_df['atr_threshold_new'] = test_df['atr'] / test_df['close'] * atr_mult
    test_df['label_long_new'] = (test_df['forward_return'] > test_df['atr_threshold_new']).astype(int)
    test_df['label_short_new'] = (test_df['forward_return'] < -test_df['atr_threshold_new']).astype(int)
    ...
    model_long = lgb.train(params, lgb.Dataset(X_train, train_df['label_long_new']), 100)
    ...
    if train_df['label_short_new'].sum() < 50:
        ...
        model_short = lgb.train(params, lgb.Dataset(X_train, train_df['label_short_new']), 100)
