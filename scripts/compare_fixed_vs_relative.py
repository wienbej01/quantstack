...
def compare_label_methods():
    ...
    for period_name, (start_date, end_date) in [
        ("Known Good (Aug 2023)", KNOWN_GOOD_PERIOD),
        ...
        if len(period_df) < 1000:
            logging.warning(
                f"Insufficient data for {period_name}: {len(period_df)} rows"
            )
            continue
        ...
        for label_type in ["fixed", "relative"]:
            ...
            model_results = train_and_test(
                train_df, test_df, feature_cols, long_col, short_col
            )
            ...
            if model_results:
                ...
                logging.info(
                    f"AUC: Long {model_results['auc_long']:.3f}, Short {model_results['auc_short']:.3f}"
                )
                logging.info(
                    f"Backtest: {model_results['trades']} trades, {model_results['win_rate']:.1f}% win, ${model_results['pnl']:,.0f}"
                )
...
def train_and_test(train_df, test_df, feature_cols, long_col, short_col):
    ...
    return {"auc_long": auc_long, "auc_short": auc_short, **backtest_results}
...
def run_simple_backtest(test_df, pred_long, pred_short, threshold=0.40):
    ...
    for _, signal in signals.iterrows():
        ...
        trades.append({"symbol": signal["symbol"], "net_pnl": net_pnl})
...
def generate_comparison_report(results_df):
    ...
    for period in results_df["period"].unique():
        ...
        if len(period_data) == 2:
            ...
            logging.info(f"\n{period}:")
            logging.info("  Model Quality (AUC Long):")
            logging.info(f"    Fixed:    {fixed['auc_long']:.3f}")
            logging.info(f"    Relative: {relative['auc_long']:.3f}")
            logging.info(
                f"    Winner:   {'Fixed' if fixed['auc_long'] > relative['auc_long'] else 'Relative'}"
            )

            logging.info("  Performance (PnL):")
            logging.info(f"    Fixed:    ${fixed['pnl']:+,.0f}")
            logging.info(f"    Relative: ${relative['pnl']:+,.0f}")
            logging.info(
                f"    Winner:   {'Fixed' if fixed['pnl'] > relative['pnl'] else 'Relative'}"
            )

            logging.info("  Label Rates (Long):")
    ...
    for period in results_df["period"].unique():
        ...
        if len(period_data) == 2:
            ...
            relative_pnl = period_data[period_data["label_type"] == "relative"][
                "pnl"
            ].iloc[0]
