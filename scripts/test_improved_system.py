...
def main():
    ...
    logging.info("\n--- LABEL COMPARISON ---")
    logging.info(
        f"Original labels - Long: {train_df['label_long'].mean()*100:.2f}%, Short: {train_df['label_short'].mean()*100:.2f}%"
    )
    logging.info(
        f"ATR labels - Long: {train_df['label_long_atr'].mean()*100:.2f}%, Short: {train_df['label_short_atr'].mean()*100:.2f}%"
    )
