...
def print_report(results: dict):
    ...
    if results["client_disconnect_summary"]:
        ...
        for reason, count in sorted(
            results["disconnect_reasons"].items(), key=lambda x: -x[1]
        ):
    ...
    if results["client_disconnects"]:
        ...
        for item in results["client_disconnects"][-10:]:
            print(
                f"  {item['timestamp']} - Client {item['client_id']}: {item['reason']}"
            )
        print()
    ...
    if results["farm_disconnects"]:
        issues.append(
            f"⚠️  Gateway lost connection to IBKR servers {len(results['farm_disconnects'])} time(s)"
        )
    ...
    for client_id, count in results["client_disconnect_summary"].items():
        if count > 10:
            issues.append(
                f"⚠️  Client {client_id} disconnected {count} times (possible connection leak)"
            )
    ...
    if "Connection terminated" in results["disconnect_reasons"]:
        ...
        if count > 20:
            issues.append(
                f"⚠️  {count} 'Connection terminated' events (possible reconnection bug)"
            )
