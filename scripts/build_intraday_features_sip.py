...
def load_sip_membership():
    ...
    logging.info(f"Loaded SIP: {len(sip)} selections, {sip['symbol'].n_unique()} symbols")
    return sip
...
def engineer_intraday_features(df, target_date):
    ...
    # Time features
    df = df.with_columns([
        ((pl.col("timestamp").dt.hour() - 9) * 60 + (pl.col("timestamp").dt.minute() - 30)).alias("time_since_open"),
        ((16 - pl.col("timestamp").dt.hour()) * 60 - pl.col("timestamp").dt.minute()).alias("time_to_close"),
    ])
...
def main():
    ...
    logging.info(f"Avg bars/symbol/day: {len(combined) / (combined['symbol'].n_unique() * combined['date'].n_unique()):.1f}")
