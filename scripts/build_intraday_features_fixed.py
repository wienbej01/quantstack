...
def engineer_clean_features(df, target_date):
    ...
    df_pd["upper_wick_pct"] = (df_pd["high"] - df_pd[["open", "close"]].max(axis=1)) / df_pd["close"]
    df_pd["lower_wick_pct"] = (df_pd[["open", "close"]].min(axis=1) - df_pd["low"]) / df_pd["close"]
    ...
    df_pd["displacement_up"] = ((df_pd["returns"] > 0) & 
                               (df_pd["returns"] > df_pd["volatility_5"] * 2)).astype(int)
    df_pd["displacement_down"] = ((df_pd["returns"] < 0) & 
                                 (abs(df_pd["returns"]) > df_pd["volatility_5"] * 2)).astype(int)
    
    # Order blocks (improved)
    df_pd["prev_bearish"] = df_pd["is_bearish"].shift(1)
    df_pd["prev_bullish"] = df_pd["is_bullish"].shift(1)
    df_pd["order_block_bull"] = ((df_pd["prev_bearish"] == 1) & 
                                (df_pd["displacement_up"] == 1)).astype(int)
    df_pd["order_block_bear"] = ((df_pd["prev_bullish"] == 1) & 
                                (df_pd["displacement_down"] == 1)).astype(int)
    ...
    df_pd["ny_open_killzone"] = ((df_pd["hour_et"] >= 9) & (df_pd["hour_et"] < 11)).astype(int)
    df_pd["ny_close_killzone"] = ((df_pd["hour_et"] >= 14) & (df_pd["hour_et"] < 16)).astype(int)
    ...
    df_pd["label_long_atr"] = (df_pd["forward_return"] > df_pd["atr_threshold"]).astype(int)
    df_pd["label_short_atr"] = (df_pd["forward_return"] < -df_pd["atr_threshold"]).astype(int)
    ...
    df_pd = df_pd[~((df_pd["hour_et"] == 15) & (df_pd["minute"] >= 30))]  # Skip 15:30-16:00
    
    # Clean up intermediate columns AND all raw price columns
    cols_to_drop = ["prev_close", "tr", "high_5", "low_5", "prev_high_5", "prev_low_5", 
                   "typical_price", "vwap_num", "vwap_den", "vwap", "up_volume", "down_volume",
                   "up_volume_5", "down_volume_5", "volume_ma5", "volume_ma20", "prev_bearish", 
                   "prev_bullish", "entry_date", "exit_date", "minute",
                   # Remove ALL raw price columns
                   "open", "high", "low", "close", "entry_close", "exit_close",
                   # Remove additional raw features from gold data
                   "first_open", "vwap_session", "prev_session_close"]
...
def main():
    logging.info("=" * 80)
    logging.info("BUILDING FIXED INTRADAY FEATURES: Timezone normalized, clean features")
    logging.info("=" * 80)
