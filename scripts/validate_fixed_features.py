...
def main():
    ...
    raw_price_cols = [c for c in pdf.columns if any(x in c.lower() for x in 
                     ['close', 'open', 'high', 'low']) and 
                     not any(x in c.lower() for x in ['pct', 'ratio', 'distance', 'position', 'killzone', 'time_', 'ret_'])]
    
    if raw_price_cols:
        logging.warning(f"Found {len(raw_price_cols)} raw price features: {raw_price_cols}")
    ...
    if extreme_features:
        ...
        for col, mean_val, std_val, max_val, min_val in extreme_features:
            logging.warning(f"  {col}: mean={mean_val:.2f}, std={std_val:.2f}, max={max_val:.2f}")
    else:
    ...
    same_day_entry = (pd.to_datetime(pdf['entry_timestamp']).dt.date == 
                     pd.to_datetime(pdf['timestamp']).dt.date).all()
    same_day_exit = (pd.to_datetime(pdf['exit_timestamp']).dt.date == 
                    pd.to_datetime(pdf['timestamp']).dt.date).all()
    ...
    logging.info(f"Afternoon rows: {len(afternoon):,} ({len(afternoon)/len(pdf)*100:.1f}%)")
    ...
    success = (
        entry_after_signal and same_day_entry and same_day_exit and
        not raw_price_issue and len(extreme_features) == 0 and
        len(morning) > len(pdf) * 0.05  # At least 5% morning data (was 10%)
    )
