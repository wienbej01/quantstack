...
def performance_by_symbol(df: pd.DataFrame):
    ...
    agg = (
        ...
        .agg({"trade_id": "count", "net_pnl": ["sum", "mean"], "winner": "sum"})
        .round(2)
    )
...
def signal_execution_analysis(df: pd.DataFrame):
    ...
    signal_df["signal_entry_slip"] = (
        signal_df["entry_price"] - signal_df["signal_entry_price"]
    )
