...
def process_existing_features():
    ...
    if 'label_long_atr' in combined_df.columns:
        ...
        logging.info(f"Labels - Original Long: {orig_long:.2f}%, ATR Long: {atr_long:.2f}%")
...
def add_improved_features(df):
    ...
    for (symbol, date), group in df.groupby(['symbol', 'date']):
        ...
        try:
            ...
            group['label_long_atr'] = (group['forward_return'] > group['atr_threshold']).astype(int)
            group['label_short_atr'] = (group['forward_return'] < -group['atr_threshold']).astype(int)
