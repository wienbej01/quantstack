#!/usr/bin/env python3
"""Build daily feature store for 3 months with checkpointing and heartbeat."""
...
def main():
    ...
    start_date = "2024-03-01"  # 3 months: Mar, Apr, May
    end_date = "2024-05-31"
    output_dir = Path("run/daily_features_3months")
    output_dir.mkdir(parents=True, exist_ok=True)
    checkpoint_dir = output_dir / "checkpoints"
    checkpoint_dir.mkdir(exist_ok=True)

    LOGGER.info("=" * 80)
    LOGGER.info("Building Daily Feature Store - 3 MONTHS with Checkpointing")
    LOGGER.info("=" * 80)
    ...
    all_features = []
    checkpoint_interval = 50  # Save every 50 symbols
    ...
    with Pool(n_workers) as pool:
        for i, result in enumerate(pool.imap_unordered(compute_daily_features_for_symbol, args_list), 1):
            ...
            if i % checkpoint_interval == 0:
                LOGGER.info(f"[CHECKPOINT] Saving intermediate results ({len(all_features)} features)...")
                if all_features:
                    ...
                    LOGGER.info(f"[CHECKPOINT] Saved batch {i}")
                
                # Update progress file
                processed_idx = i - 1
                if processed_idx < len(remaining_symbols):
                    with open(checkpoint_file, "a") as f:
                        for j in range(max(0, processed_idx - checkpoint_interval + 1), processed_idx + 1):
