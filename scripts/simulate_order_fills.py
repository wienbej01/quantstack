            ...
            if '<- [3;' in line and ';IOC;' in line:
                try:
                    ...
                    if hour >= 22:  # 22:00+ Manila = 09:00+ ET same day
                        timestamp = et_tz.localize(datetime(2026, 1, 23, hour - 13, int(m), int(s), int(ms) * 1000))
                    else:  # 00:00-05:00 Manila = 11:00-16:00 ET previous day
                        timestamp = et_tz.localize(datetime(2026, 1, 23, hour + 11, int(m), int(s), int(ms) * 1000))
...
def main():
    ...
    if results['total'] > 0:
        print(f"Filled at order price: {results['at_order']} ({100*results['at_order']/results['total']:.1f}%)")
        print(f"Would fill at -1 tick: {results['minus_1']} ({100*results['minus_1']/results['total']:.1f}%)")
        print(f"Would fill at +1 tick: {results['plus_1']} ({100*results['plus_1']/results['total']:.1f}%)")
        print(f"No fill even at +/-1: {results['no_fill']} ({100*results['no_fill']/results['total']:.1f}%)")
        print(f"No L2 data available: {results['no_data']} ({100*results['no_data']/results['total']:.1f}%)")
