...
def backtest_model(model_long, model_short, df_pd, feature_cols, threshold=0.30):
    ...
    long_wins = (
        (long_signals["forward_return"] > 0.015).sum() if len(long_signals) > 0 else 0
    )
    short_wins = (
    ...
    long_total_return = (
        long_signals["forward_return"].sum() if len(long_signals) > 0 else 0
    )
    short_total_return = (
        -short_signals["forward_return"].sum() if len(short_signals) > 0 else 0
    )
    total_pnl = long_total_return + short_total_return

    long_avg_return = (
        long_signals["forward_return"].mean() if len(long_signals) > 0 else 0
    )
    short_avg_return = (
        -short_signals["forward_return"].mean() if len(short_signals) > 0 else 0
    )
...
def main():
    ...
    logging.info("=" * 80)
    metrics_13 = backtest_model(
        model_13_long, model_13_short, df_13_pd.copy(), features_13
    )
    ...
    logging.info("=" * 80)
    metrics_30 = backtest_model(
        model_30_long, model_30_short, df_30_pd.copy(), features_30
    )
    ...
    improvements = {
        "Win Rate": (metrics_30["combined_win_rate"] - metrics_13["combined_win_rate"])
        * 100,
