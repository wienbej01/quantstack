...
from extensions.intraday_ml.backtest_bt_1m import run_backtest_1m
...
def main():
    ...
    print("="*80)
    print("MAY 2024 BACKTEST - 1-MINUTE EXECUTION + EOD CLOSE FIX")
    print("="*80)
    ...
    config = {
        ...
        'costs': {'per_share': 0.0035, 'commission_min': 0.35},
    }
    ...
    # Run backtest
    results, trades_df = run_backtest_1m(bars_1m, orders, config)
    ...
    if not trades_df.empty:
        ...
        print("\n--- Duration ---")
        print(f"Max:    {trades_df['duration_minutes'].max():.0f} minutes ({trades_df['duration_minutes'].max()/60:.1f} hours)")
        ...
        if trades_df['actual_move'].mean() > 0:
            print(f"Commission vs move: {(trades_df['commission'].mean() / trades_df['actual_move'].mean() * 100):.0f}%")
        
        print("\n--- Validation ---")
        max_dur = trades_df['duration_minutes'].max()
        print(f"✓ EOD close working: {'PASS' if max_dur < 400 else f'FAIL (max {max_dur:.0f}m)'}")
        
        target_pct = (trades_df['exit_reason'] == 'TARGET').sum() / len(trades_df) * 100
        print(f"✓ Target hits > 10%: {'PASS' if target_pct > 10 else f'FAIL ({target_pct:.1f}%)'}")
        
        win_rate = results.get('win_rate', 0)
        print(f"✓ Win rate > 40%: {'PASS' if win_rate > 40 else f'CHECK ({win_rate:.1f}%)'}")
        
        zero_moves = (trades_df['actual_move'] < 0.001).sum()
        print(f"✓ Zero moves < 5%: {'PASS' if zero_moves/len(trades_df) < 0.05 else f'CHECK ({zero_moves/len(trades_df)*100:.1f}%)'}")
