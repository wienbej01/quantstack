...
def engineer_features(df):
    ...
    df = df.with_columns([
        ((pl.col("high") - pl.col("candle_top")) / pl.col("close")).alias("upper_wick"),
        ((pl.col("candle_bottom") - pl.col("low")) / pl.col("close")).alias("lower_wick"),
    ])
    ...
    df = df.with_columns([
        ((pl.col("close") - pl.col("low_5")) / (pl.col("high_5") - pl.col("low_5") + 1e-8)).alias("price_position"),
    ])
    ...
    # ICT: Displacement
    df = df.with_columns([
        (pl.col("returns").abs() > (pl.col("volatility_5") * 2)).cast(pl.Int8).alias("displacement"),
    ])
    df = df.with_columns([
        ((pl.col("returns") > 0) & (pl.col("displacement") == 1)).cast(pl.Int8).alias("displacement_up"),
        ((pl.col("returns") < 0) & (pl.col("displacement") == 1)).cast(pl.Int8).alias("displacement_down"),
    ])
    ...
    df = df.with_columns([
        (pl.col("volume_ratio") / (pl.col("range_pct") + 1e-8)).alias("effort_result_ratio"),
    ])
    
    # ATR-based features
    df = df.with_columns([
        (pl.col("atr14") / pl.col("close")).alias("atr_pct"),
        (pl.col("range_pct") / (pl.col("atr14") / pl.col("close") + 1e-8)).alias("range_atr_ratio"),
    ])
...
def create_labels(df, forward_periods=5, profit_threshold=0.015):
    ...
    # Forward returns
    df = df.with_columns([
        pl.col("close").shift(-forward_periods).over("symbol").alias("future_close"),
    ])
    df = df.with_columns([
...
def main():
    ...
    feature_cols = [
        ...
        "price_position",
        "gap_pct", "atr_pct", "range_atr_ratio",
        "fvg_up", "fvg_down",
