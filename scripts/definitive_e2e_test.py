...
def test_no_mock_config():
    ...
    for f in config_dir.glob('*.yaml'):
        content = f.read_text()
        assert 'mock_data' not in content.lower() or 'enabled: false' in content.lower(), \
...
def test_mock_guard():
    ...
    assert 'mock_data' in main_py and 'RuntimeError' in main_py, \
...
def test_import_order_manager():
    from execution.order_manager import IBKROrderManager, OrderRequest, OrderSide, OrderType, OrderUpdate
...
def test_import_signals():
    from signals.l2_signals import L2SignalGenerator, SignalType, TradingSignal, SignalValidator
...
def test_import_risk():
    from risk.risk_manager import RiskManager, RiskStatus, Position, RiskMetrics
...
SNAPSHOT = L2Snapshot(
    symbol='BE', timestamp=1234567890.0, bid=10.0, ask=10.02, mid=10.01,
    spread=0.02, bid_size=1000, ask_size=800, obi_1=0.111, obi_5=0.15,
    depth_bid=5000.0, depth_ask=4000.0, pressure=0.2, d_mid_5s=0.001, d_obi_1_5s=0.02
)
...
@test("RiskManager.calculate_position_size(symbol, strength, conf, account, price) signature")
...
@test("TradeJournal.record_trade_entry(symbol, side, quantity, entry_price, order_id) signature")
...
def test_svc_collector():
    result = subprocess.run(['systemctl', 'is-active', 'l2-collector'], capture_output=True, text=True)
    assert result.stdout.strip() == 'active', f"l2-collector: {result.stdout.strip()}"


@test("l2-scalping service active")
def test_svc_scalping():
    result = subprocess.run(['systemctl', 'is-active', 'l2-scalping'], capture_output=True, text=True)
    assert result.stdout.strip() == 'active', f"l2-scalping: {result.stdout.strip()}"


@test("l2-watchdog service active")
def test_svc_watchdog():
    result = subprocess.run(['systemctl', 'is-active', 'l2-watchdog'], capture_output=True, text=True)
    assert result.stdout.strip() == 'active', f"l2-watchdog: {result.stdout.strip()}"


@test("trading-orchestrator timer active")
def test_timer_orch():
    result = subprocess.run(['systemctl', 'is-active', 'trading-orchestrator.timer'], capture_output=True, text=True)
    assert result.stdout.strip() == 'active', f"trading-orchestrator.timer: {result.stdout.strip()}"
