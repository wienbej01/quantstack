...
class TestRunner:
    ...
    def run_query(self, query, params=None):
        try:
            ...
            if params is not None:
                ...
                cur.execute(query)
            if query.strip().upper().startswith("SELECT") or query.strip().upper().startswith("EXPLAIN"):
                return cur.fetchall()
    ...
    def test_1_1_single_trade(self, result):
        ...
        # Verify
        trade = self.run_query("SELECT * FROM trades_v2 WHERE trade_id = %s", (trade_id,))
        if not trade:
            result.add_error("Trade not created")
            return
            
        fills = self.run_query("SELECT COUNT(*) FROM executions WHERE trade_id = %s", (trade_id,))
        if fills[0][0] != 2:
            result.add_error(f"Expected 2 fills, got {fills[0][0]}")
            
        status = self.run_query("SELECT status FROM trades_v2 WHERE trade_id = %s", (trade_id,))
        if status and status[0][0] != 'CLOSED':
            result.add_error(f"Status should be 'CLOSED', got '{status[0][0]}'")
            
        pnl = self.run_query("SELECT net_pnl FROM trades_v2 WHERE trade_id = %s", (trade_id,))
    ...
    def test_1_2_partial_fills(self, result):
        ...
        # Verify
        trade = self.run_query("SELECT entry_price, entry_qty, entry_fills FROM trades_v2 WHERE trade_id = %s", (trade_id,))
        if not trade:
        ...
        if abs(actual_vwap - expected_vwap) > 0.01:
            result.add_error(f"VWAP should be {expected_vwap:.2f}, got {actual_vwap:.2f}")
    ...
    def test_1_3_deduplication(self, result):
        ...
        # Verify only 1 row
        count = self.run_query("SELECT COUNT(*) FROM executions WHERE exec_id = %s", (exec_id,))
        if count[0][0] != 1:
            result.add_error(f"Should have 1 row, got {count[0][0]}")
            
        # Verify first source wins
        source = self.run_query("SELECT source FROM executions WHERE exec_id = %s", (exec_id,))
            ...
            for line in f:
                try:
                    ...
                    self.run_query("""
                        ...
                        ON CONFLICT (exec_id) DO NOTHING
                    """, (fill['exec_id'], fill['symbol'], 'test', fill['side'], fill['qty'], 
                          fill['price'], fill['commission'], fill['exec_time'], source))
                    recovered += 1
    ...
    def test_3_1_concurrent_inserts(self, result):
        ...
        count = self.run_query("SELECT COUNT(*) FROM executions WHERE exec_id = 'CONC123'")
        if count[0][0] != 1:
            result.add_error(f"Should have 1 row after concurrent inserts, got {count[0][0]}")
    ...
    def test_4_1_pnl_accuracy(self, result):
        ...
        trade = self.run_query("SELECT gross_pnl, total_commission, net_pnl FROM trades_v2 WHERE trade_id = %s", (trade_id,))
    ...
    def test_5_1_throughput(self, result):
        ...
        try:
            ...
            # Verify all inserted
            count = self.run_query("SELECT COUNT(*) FROM trades_v2 WHERE symbol LIKE 'STRESS%'")
    ...
    def test_6_1_position_aggregation(self, result):
        ...
        # Verify
        pos = self.run_query("SELECT quantity, avg_price FROM positions WHERE symbol = 'TEST7'")
        if not pos:
    ...
    def test_6_2_reconciliation_status(self, result):
        ...
        pos = self.run_query("SELECT last_reconcile, is_reconciled FROM positions WHERE symbol = 'TEST7'")
        if not pos:
        ...
        # Verify
        pos = self.run_query("SELECT is_reconciled FROM positions WHERE symbol = 'TEST7'")
        if not pos[0][0]:
