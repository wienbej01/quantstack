...
def main():
    ...
    tests = [
        ...
        ("SIP file exists", lambda: assert_true(Path('/home/jacobw/intraday_stack/data/daily_sip').glob('date=*'))),
    ...
    if ERRORS:
        msg = f"Pre-flight FAILED at {datetime.now().strftime('%H:%M ET')}:\n" + "\n".join(ERRORS[:5])
        ...
        if os.environ.get('PREFLIGHT_NOTIFY_SUCCESS'):
            send_ntfy("✅ Pre-Flight OK", "System ready for trading", priority="low", tags="white_check_mark")
        return 0
