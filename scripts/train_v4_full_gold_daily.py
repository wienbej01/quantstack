...
def engineer_features(df):
    ...
    df = df.with_columns(
        [
            ((pl.col("close") - pl.col("prior_close")) / pl.col("prior_close")).alias(
                "daily_return"
            ),
    ...
    df = df.with_columns(
        [
            (pl.col("daily_return").rolling_std(5).over("symbol")).alias(
                "volatility_5d"
            ),
            (pl.col("daily_return").rolling_std(10).over("symbol")).alias(
                "volatility_10d"
            ),
        ]
    )
    ...
    df = df.with_columns(
        [
            ...
            (
                ...
                / (pl.col("atr14") / pl.col("close") + 1e-8)
            ).alias("range_atr_ratio"),
        ]
    )
