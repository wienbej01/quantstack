...
def load_monthly_bars(
    symbol, year, month, data_root="/home/jacobw/gcs-mount/gold/stocks/1m"
):
...
def main():
    ...
    for year_month in [(2025, 9), (2025, 10), (2025, 11), (2025, 12)]:
        ...
        for i, symbol in enumerate(symbols):
            ...
            symbol_dates = month_sip.filter(pl.col("symbol") == symbol)[
                "date"
            ].to_list()
        ...
        if month_features:
            ...
            logging.info(
                f"✅ {year}-{month:02d}: {len(month_df):,} feature rows from {len(month_features)} symbols"
            )
        else:
