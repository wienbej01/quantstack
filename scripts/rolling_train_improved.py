...
def train_models(train_df, val_df, feature_cols):
    ...
    model_long = lgb.train(
        params, train_data, num_boost_round=300,  # Reduced to prevent overfitting
        valid_sets=[val_data],
...
def main():
    ...
    if not features_path.exists():
        logging.error("Improved features not found. Run build_intraday_features_improved.py first")
        return
    ...
    exclude_cols = [
        ...
        "open", "high", "low", "close", "vwap_session", "prev_session_close",
        "first_open", "prev_close", "atr", "tr", "volume", "cum_volume", "cum_dollar_vol",
    ]
    ...
    while current < end_date:
        ...
        logging.info(f"Train: {train_start.strftime('%Y-%m')} to {train_end.strftime('%Y-%m')}")
        logging.info(f"Val: {val_start.strftime('%Y-%m')} to {oos_start.strftime('%Y-%m')}")
        ...
        logging.info(f"Data: Train {len(train_df):,} | Val {len(val_df):,} | OOS {len(test_df):,}")
        ...
        # Backtest
        trades_df, equity = backtest_improved(model_long, model_short, test_df, feature_cols, equity)
        ...
        if trades_df is not None and len(trades_df) > 0:
            ...
            logging.info(f"Results: {n_trades} trades | {unique_symbols} symbols | {win_rate:.1f}% win | ${total_pnl:,.0f} PnL")
    ...
    if all_trades:
        ...
        logging.info(f"Profitable months: {profitable_months}/{total_months} ({profitable_months/total_months*100:.0f}%)")
        ...
        logging.info(f"Top symbol: {symbol_counts.index[0]} ({symbol_counts.iloc[0]} trades)")
