...
def check_system_status():
    ...
    sip_dir = Path('data/daily_sip')
    
    sip_file = sip_dir / f'sip_universe_{date_str}.txt'
    l2_file = sip_dir / f'l2_symbols_{date_str}.txt'
    
    if sip_file.exists() and l2_file.exists():
        with open(sip_file) as f:
            sip_count = len([line for line in f if line.strip()])
        with open(l2_file) as f:
            l2_symbols = [line.strip() for line in f if line.strip()]
        
        print(f"✅ Daily SIP: {sip_count} symbols selected")
        print(f"✅ L2 Symbols: {l2_symbols}")
    else:
        print("❌ No daily SIP results found")
        print("   Run: make daily-sip")
    ...
    if l2_dir.exists():
        ...
        if l2_runs:
            ...
            print(f"✅ L2 Data: {len(raw_dirs)} symbol collections in {latest_run.name}")
        else:
