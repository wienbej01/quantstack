...
def load_daily_features():
    ...
    return daily_df.select(["date", "symbol", "market_vol_5d", "market_trend_5d", "avg_gap_5d"])
...
def calculate_ict_vpa_features(df_pd):
    ...
    df_pd["price_vs_vwap"] = (df_pd["close"] - df_pd["vwap_session"]) / df_pd["vwap_session"]
    ...
    df_pd["upper_wick"] = (df_pd["high"] - np.maximum(df_pd["open"], df_pd["close"])) / df_pd["close"]
    df_pd["lower_wick"] = (np.minimum(df_pd["open"], df_pd["close"]) - df_pd["low"]) / df_pd["close"]
    
    # Volume features (relative to session average)
    df_pd["volume_vs_avg"] = df_pd["volume"] / df_pd.groupby("session_id")["volume"].transform("mean")
    df_pd["dollar_vol_vs_avg"] = df_pd["cum_dollar_vol"] / df_pd.groupby("session_id")["cum_dollar_vol"].transform("mean")
    
    # Time-based features
    df_pd["minutes_from_open"] = (df_pd["timestamp"] - df_pd.groupby("session_id")["timestamp"].transform("min")).dt.total_seconds() / 60
    df_pd["minutes_to_close"] = 390 - df_pd["minutes_from_open"]  # 6.5 hours = 390 minutes
    ...
    for period in [3, 5, 10, 20]:
        df_pd[f"return_{period}m"] = df_pd.groupby("session_id")["close"].pct_change(period)
        df_pd[f"high_{period}m"] = df_pd.groupby("session_id")["high"].rolling(period).max() / df_pd["close"] - 1
        df_pd[f"low_{period}m"] = df_pd.groupby("session_id")["low"].rolling(period).min() / df_pd["close"] - 1
    ...
    df_pd["up_volume_pct"] = df_pd["up_volume"] / (df_pd["up_volume"] + df_pd["down_volume"] + 1e-8)
    df_pd["volume_imbalance"] = (df_pd["up_volume"] - df_pd["down_volume"]) / (df_pd["up_volume"] + df_pd["down_volume"] + 1e-8)
...
def calculate_labels_atr_normalized(df_pd):
    ...
    df_pd["forward_return"] = df_pd.groupby("session_id")["close"].pct_change(-FORWARD_BARS)
    ...
    df_pd["exit_timestamp"] = df_pd.groupby("session_id")["timestamp"].shift(-FORWARD_BARS-1)
...
def add_regime_features(df_pd, daily_df):
    ...
    # Fill missing values
    df_pd["market_vol_5d"] = df_pd["market_vol_5d"].fillna(df_pd["market_vol_5d"].median())
    df_pd["market_trend_5d"] = df_pd["market_trend_5d"].fillna(0)
    df_pd["avg_gap_5d"] = df_pd["avg_gap_5d"].fillna(df_pd["avg_gap_5d"].median())
    
    # Regime indicators
    df_pd["high_vol_regime"] = (df_pd["market_vol_5d"] > df_pd["market_vol_5d"].quantile(0.7)).astype(int)
    df_pd["bull_regime"] = (df_pd["market_trend_5d"] > 0.02).astype(int)  # >2% 5-day return
    df_pd["bear_regime"] = (df_pd["market_trend_5d"] < -0.02).astype(int)  # <-2% 5-day return
...
def main():
    ...
    for row in sip_df.iter_rows(named=True):
        ...
        if features is not None:
            ...
            if processed % 50 == 0:
                logging.info(f"Processed {processed:,} symbol-dates, skipped {skipped:,}")
        else:
            skipped += 1
    ...
    price_features = [c for c in feature_cols if any(x in c.lower() for x in ["open", "high", "low", "close", "vwap_session", "prev_session_close"])]
    if price_features:
