...
store = EventStore(use_postgres=True, pg_config={'database': 'trading', 'user': 'jacobw'})
...
if test_trades:
    t = test_trades[-1]
    print(f"  Trade: {t['symbol']} {t['direction']} {t['entry_qty']}@{t['entry_price']} -> {t['exit_price']}")
    print(f"  P&L: ${t['net_pnl']:.2f} (gross: ${t['gross_pnl']:.2f}, commission: ${t['commission']:.2f})")
