...
def main():
    ...
    # Drop nulls
    df = df.drop_nulls()
    logging.info(f"Data: {len(df):,} bars, {df['symbol'].n_unique()} symbols, {df['date'].n_unique()} dates")
    ...
    logging.info(f"LONG signals: {(df_pd['prediction'] == 1).sum():,} ({(df_pd['prediction'] == 1).mean():.2%})")
    logging.info(f"SHORT signals: {(df_pd['prediction'] == -1).sum():,} ({(df_pd['prediction'] == -1).mean():.2%})")
    logging.info(f"Neutral: {(df_pd['prediction'] == 0).sum():,} ({(df_pd['prediction'] == 0).mean():.2%})")
    ...
    # Combined
    combined_wins = (
        (long_signals["forward_return"] > 0.015).sum() +
        (short_signals["forward_return"] < -0.015).sum()
    )
    ...
    df_pred = pl.from_pandas(df_pd[["timestamp", "symbol", "date", "close", "prob_long", "prob_short", "prediction", "forward_return"]])
    df_pred.write_parquet(output_dir / "predictions.parquet")
    logging.info(f"\nSaved predictions to: {output_dir}/predictions.parquet")
    
    # Save signals only
    signals_pl = pl.from_pandas(signals[["timestamp", "symbol", "date", "close", "prob_long", "prob_short", "prediction", "forward_return"]])
