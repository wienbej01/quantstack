...
def get_fills(db_path: str, trade_id: str) -> List[Dict]:
    ...
    cursor.execute(
        "SELECT * FROM fills WHERE trade_id = ? ORDER BY timestamp", (trade_id,)
    )
...
def print_report(trades: List[Dict], db_path: str, date: str = None):
    ...
    for key, s in stats.items():
        ...
        print(
            f"  Trades: {s['total_trades']} (Open: {s['open_trades']}, Closed: {s['closed_trades']})"
        )
    ...
    for trade in trades:
        ...
        pnl_icon = (
            ...
            else "❌" if (trade["net_pnl"] or 0) < 0 else "⚪"
        )
        system = trade.get("system", "unknown")
        ...
        print(
            f"   Entry: {trade['entry_time']} @ ${trade['entry_price']:.2f} x {trade['entry_qty']}"
        )

        if trade["status"] == "CLOSED":
            print(
                f"   Exit:  {trade['exit_time']} @ ${trade['exit_price']:.2f} x {trade['exit_qty']}"
            )
            print(f"   Reason: {trade['exit_reason']}")
            print(f"   Hold: {trade['hold_time_seconds']:.0f}s")
            print(
                f"   {pnl_icon} Net P&L: ${trade['net_pnl']:,.2f} (Gross: ${trade['gross_pnl']:,.2f}, Comm: ${trade['commission']:,.2f})"
            )
        else:
            print(f"   Status: OPEN")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Event store database path",
    )
