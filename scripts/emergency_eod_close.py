...
def emergency_close_positions():
    ...
    if now_et.time().hour < 15 or (now_et.time().hour == 15 and now_et.time().minute < 50):
    ...
    logger.error(f"⚠️  EMERGENCY: {len(open_trades)} OPEN POSITIONS FOUND AFTER 3:50 PM ET")
    
    for trade in open_trades:
        logger.error(f"  OPEN: {trade['trade_id']} | {trade['symbol']} | {trade['system']} | "
    ...
    for trade in open_trades:
        ...
        logger.warning(f"FORCE CLOSING: {trade['symbol']} (trade_id={trade['trade_id']})")
    ...
    logger.error(f"⚠️  EMERGENCY CLOSE COMPLETE: {len(open_trades)} positions force-closed")
    ...
    try:
        ...
        requests.post(
            ...
            data=f"EMERGENCY EOD: Force closed {len(open_trades)} positions after IBKR failure".encode('utf-8'),
