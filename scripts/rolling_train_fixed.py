...
def get_feature_columns(df):
    ...
    exclude = ['date', 'symbol', 'timestamp', 'entry_timestamp', 'exit_timestamp',
               'label_long_atr', 'label_short_atr', 'entry_close', 'exit_close',
               'forward_return', 'atr_threshold', 'hour_et']
...
def train_time_stratified_models(train_df, val_df, feature_cols, time_period="morning"):
    ...
    if len(train_filtered) < 1000 or len(val_filtered) < 100:
        logging.warning(f"Insufficient {time_period} data: train={len(train_filtered)}, val={len(val_filtered)}")
    ...
    if y_train_long.sum() >= 50 and y_val_long.sum() >= 10:
        ...
        model_long = lgb.train(params, train_ds, num_boost_round=300,
    ...
    if y_train_short.sum() >= 50 and y_val_short.sum() >= 10:
        ...
        model_short = lgb.train(params, train_ds, num_boost_round=300,
                               valid_sets=[val_ds], callbacks=[lgb.early_stopping(30), lgb.log_evaluation(0)])
        auc_short = roc_auc_score(y_val_short, model_short.predict(X_val))
    
    logging.info(f"{time_period.title()} models - Long AUC: {auc_long:.3f}, Short AUC: {auc_short:.3f}")
...
def backtest_time_stratified(models, test_df, feature_cols, equity=EQUITY):
    ...
    for _, signal in signals.iterrows():
        ...
        shares = int(risk_amount / (signal['entry_close'] * stop_distance))
        ...
        # Simple exit at fixed bars (can be enhanced with stop/target monitoring)
...
def main():
    ...
    if not features_path.exists():
        logging.error("Fixed features not found. Run build_intraday_features_fixed.py first")
        return
    ...
    while current_date <= end_date:
        ...
        morning_long, morning_short, morning_auc_l, morning_auc_s = train_time_stratified_models(
            train_df, val_df, feature_cols, "morning")
        afternoon_long, afternoon_short, afternoon_auc_l, afternoon_auc_s = train_time_stratified_models(
            train_df, val_df, feature_cols, "afternoon")
        ...
        logging.info(f"Trades: {len(trades)}, Win Rate: {win_rate:.1%}, "
