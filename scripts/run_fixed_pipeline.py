...
def main():
    ...
    success = run_script(
        "build_intraday_features_fixed.py",
    ...
    # Validation
    success = run_script(
        "validate_fixed_features.py", "Validation: Check feature quality"
    )
    if not success:
    ...
    success = run_script(
        "rolling_train_fixed.py", "Phase 3: Time-stratified training with morning focus"
    )
    if not success:
