#!/usr/bin/env python3
"""Generate SIP membership for 2024-2025 from daily features."""
...
def main():
    logging.info("=" * 80)
    logging.info("GENERATING SIP MEMBERSHIP: 2024-2025")
    logging.info("=" * 80)
    
    # Load daily features
    features_path = Path("run/daily_features_2024_2025/features.parquet")
    ...
    # Save
    output_dir = Path("run/sip_membership_2024_2025")
    ...
    # Daily distribution
    daily_counts = sip.group_by("date").agg(pl.len().alias("count"))
    logging.info("")
    ...
    # Top symbols
    top_symbols = sip.group_by("symbol").agg(pl.len().alias("count")).sort("count", descending=True).head(20)
    logging.info("")
