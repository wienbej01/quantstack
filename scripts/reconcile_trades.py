...
def reconcile_trade(trade: dict, ibkr_fills: dict, audit: dict) -> dict:
    ...
    if not entry_order_id:
        ...
        result["issues"].append(
            f"NO_IBKR_ENTRY_FILLS: order_id={entry_order_id} not found in IBKR log"
        )
    else:
        ...
        signal_price = (
            float(trade["signal_entry_price"]) if trade["signal_entry_price"] else None
        )
        ...
        if ibkr_qty != db_entry_qty:
            result["issues"].append(
                f"ENTRY_QTY_MISMATCH: DB={db_entry_qty} IBKR={ibkr_qty}"
            )
    ...
    if trade["exit_time"]:
        ...
        if exit_order_id and not exit_fills:
            ...
            if ibkr_exit_qty != db_exit_qty:
                result["issues"].append(
                    f"EXIT_QTY_MISMATCH: DB={db_exit_qty} IBKR={ibkr_exit_qty}"
                )
...
def find_ibkr_log(
    date_str: str, ibkr_log: str = None, ibkr_dir: str = None
) -> str | None:
...
def run_reconciliation(
    date_str: str, ibkr_log: str = None, ibkr_dir: str = None, save: bool = True
):
    ...
    if passed > 0:
        ...
        for r in results:
            if r["status"] == "PASS":
                pnl_str = (
                    f"PnL=${r.get('pnl_db', 0):.2f}"
                    if r.get("pnl_db") is not None
                    else ""
                )
                print(
                    f"  ✓ [{r['trade_id_short']}] {r['symbol']} {r['direction']} {pnl_str}"
                )
    ...
    for oid in orphan_orders:
        ...
        if fills and any(
            "SCALP" in f.get("order_ref", "")
            or "reversal" in f.get("order_ref", "")
            or "ENTRY" in f.get("order_ref", "")
            for f in fills
        ):
    ...
    if trading_orphans:
        print(
            f"  ⚠️  ORPHAN IBKR ORDERS (fills with no DB trade): {len(trading_orphans)}"
        )
        for o in trading_orphans[:10]:
            print(
                f"      order_id={o['order_id']} {o['symbol']} {o['side']} qty={o['total_qty']}"
            )
    else:
    ...
    print(
        f"  {'✓' if audit_open_pct == 100 else '⚠️'} Audit OPEN coverage: {audit_open_pct:.1f}% ({audit_open_count}/{len(results)})"
    )
    print(
        f"  {'✓' if audit_close_pct == 100 else '⚠️'} Audit CLOSE coverage: {audit_close_pct:.1f}% ({audit_close_count}/{closed_trades})"
    )
    ...
    if price_checked:
        print(
            f"  {'✓' if price_matches == price_checked else '⚠️'} Entry price accuracy: {price_matches}/{price_checked} trades match IBKR VWAP"
        )
    ...
    if pnl_checked:
        print(
            f"  {'✓' if pnl_matches == pnl_checked else '⚠️'} PnL accuracy: {pnl_matches}/{pnl_checked} trades verified"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ibkr-dir", help="Base directory for IBKR logs (default: ~/IBKRlogs)"
    )
    parser.add_argument(
