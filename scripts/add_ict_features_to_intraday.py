...
def add_ict_features(df):
    ...
    # ICT: Liquidity Grabs
    df_pd["high_5"] = df_pd.groupby("symbol")["high"].transform(lambda x: x.rolling(5, min_periods=1).max())
    df_pd["low_5"] = df_pd.groupby("symbol")["low"].transform(lambda x: x.rolling(5, min_periods=1).min())
    ...
    df_pd["up_volume_5"] = df_pd.groupby("symbol")["up_volume"].transform(lambda x: x.rolling(5, min_periods=1).sum())
    df_pd["down_volume_5"] = df_pd.groupby("symbol")["down_volume"].transform(lambda x: x.rolling(5, min_periods=1).sum())
    df_pd["pressure_ratio"] = df_pd["up_volume_5"] / (df_pd["down_volume_5"] + 1)
    
    # VPA: VWAP
    df_pd["typical_price"] = (df_pd["high"] + df_pd["low"] + df_pd["close"]) / 3
    df_pd["vwap_num"] = df_pd.groupby("symbol")["typical_price"].transform(lambda x: (x * df_pd.loc[x.index, "volume"]).rolling(20, min_periods=1).sum())
    df_pd["vwap_den"] = df_pd.groupby("symbol")["volume"].transform(lambda x: x.rolling(20, min_periods=1).sum())
    df_pd["vwap"] = df_pd["vwap_num"] / (df_pd["vwap_den"] + 1)
    df_pd["distance_from_vwap"] = (df_pd["close"] - df_pd["vwap"]) / df_pd["vwap"]
    
    # VPA: Volume Momentum
    df_pd["volume_momentum"] = df_pd.groupby("symbol")["volume"].transform(lambda x: x.pct_change(5))
    
    # VPA: Price-Volume Divergence
    df_pd["price_change_5"] = df_pd.groupby("symbol")["close"].transform(lambda x: x.pct_change(5))
    df_pd["volume_change_5"] = df_pd.groupby("symbol")["volume"].transform(lambda x: x.pct_change(5))
