...
def calculate_improved_features(df_pd):
    ...
    df_pd["minutes_from_open"] = (
        df_pd["timestamp"] - df_pd["timestamp"].iloc[0]
    ).dt.total_seconds() / 60
...
def main():
    ...
    for (symbol, date), group in df_pd.groupby(["symbol", "date"]):
        ...
        try:
            ...
            target_date = (
                ...
                if not isinstance(date, type(pd.to_datetime("2025-01-01").date()))
                else date
            )
    ...
    price_features = [
        ...
        if any(
            x in c.lower() for x in ["vwap_session", "first_open", "prev_session_close"]
        )
    ]
    if price_features:
