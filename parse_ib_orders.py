...
with open('/home/jacobw/api-exported-logs.txt', 'r') as f:
    for line in f:
        if '<- [3;' in line and 'IOC' in line:
            match = re.search(r'(\d{2}:\d{2}:\d{2}:\d{3}).*<- \[3;(\d+);.*?;([A-Z]+);.*?;(BUY|SELL);(\d+);(LMT|MKT);([\d.]+)', line)
            if match:
...
print("\nFirst 10 orders:")
for o in orders[:10]:
    print(f"  {o['time']} | {o['action']:4} {o['symbol']:6} @ ${o['price']:.2f} | Order #{o['order_id']}")
...
if 'NVDA' in by_symbol:
    ...
    print(f"\nNVDA price analysis:")
    if buy_prices:
        print(f"  BUY orders: {len(buy_prices)}, range ${min(buy_prices):.2f} - ${max(buy_prices):.2f}")
    if sell_prices:
        print(f"  SELL orders: {len(sell_prices)}, range ${min(sell_prices):.2f} - ${max(sell_prices):.2f}")
