...
def wf(
    ...
    plan: pathlib.Path = typer.Option(..., "--plan", help="Walk-forward plan YAML"),
    name: str = typer.Option(..., "--name", help="Experiment ID"),
) -> None:
    """Run walk-forward test with specified plan."""
    console.print(f"Running wf experiment: {name}")

    # Load plan
    with open(plan) as f:
        wf_plan = yaml.safe_load(f)
    ...
    # For each fold in plan, run backtest
    run_ids = []
    for fold in wf_plan.get("folds", []):
        config = {**base_config, **fold}
