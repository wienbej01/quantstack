...
@app.command("regime-slice")
def regime_slice(
    cfg: pathlib.Path = typer.Option(..., "--cfg", help="Base config file"),
    regimes: str = typer.Option(..., "--regimes", help="Regime dimensions, e.g., vol_tercile,session,dow"),
    name: str = typer.Option(..., "--name", help="Experiment ID"),
) -> None:
    """Run regime slice test across specified dimensions."""
    console.print(f"Running regime-slice experiment: {name}")
    ...
    # For each regime, run backtest
    run_ids = []
    for regime in regime_list:
        config = {**base_config, "regime": regime}
