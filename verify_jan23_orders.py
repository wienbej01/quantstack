...
def check_order_vs_market(order, l2_data):
    ...
    if action == "BUY":
        ...
        result["reason"] = (
            f"Order @ {order_price:.2f}, Ask @ {ask:.2f}, Diff: {result['price_vs_ask']:.3f}"
        )
    else:  # SELL
        ...
        result["reason"] = (
            f"Order @ {order_price:.2f}, Bid @ {bid:.2f}, Diff: {result['price_vs_bid']:.3f}"
        )
...
def main():
    ...
    if should_fill > 0:
        ...
        print(
            df_results[df_results["should_fill"]][
                [
                    "order_time",
                    "action",
                    "order_price",
