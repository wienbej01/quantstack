...
def _compute_first_hour_avwap(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def compute_group_first_hour_avwap(group):
        ...
        is_first_hour = (group["ts"] >= group["session_start_ns"]) & (group["ts"] <= group["first_hour_ns"])
...
def _compute_prev_extreme_avwaps(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def compute_group_prev_extremes(group):
        ...
        daily_extremes = group.groupby("session_date").agg({
            "high": "max",
            "low": "min"
        ...
        touches_prev_high = (
            ((group["high"] >= prev_high) & high_touch_allowed) | fallback_high_trigger
        )
        touches_prev_low = (
            ((group["low"] <= prev_low) & low_touch_allowed) | fallback_low_trigger
        )
        ...
        group["vol_high_cumsum"] = np.where(
            touches_prev_high,
            group["volume"].groupby(high_anchor_ids).cumsum(),
            np.nan
        )
        ...
        group["vol_low_cumsum"] = np.where(
            touches_prev_low,
            group["volume"].groupby(low_anchor_ids).cumsum(),
            np.nan
        )
        ...
        high_series = pd.Series(high_avwap, index=group.index).groupby(
            [group["session_date"], high_anchor_ids]
        ).ffill()
        low_series = pd.Series(low_avwap, index=group.index).groupby(
            [group["session_date"], low_anchor_ids]
        ).ffill()
    ...
    df["f__anchor__prev_high_avwap"] = extreme_avwaps["f__anchor__prev_high_avwap"].reset_index(drop=True)
    df["f__anchor__prev_low_avwap"] = extreme_avwaps["f__anchor__prev_low_avwap"].reset_index(drop=True)
...
def compute_intraday_volume_profile(
    ...
    def compute_group_profile(group):
        ...
        for i in range(n):
            ...
            for j, (idx_lo, idx_hi, vol) in enumerate(zip(idx_lo_all, idx_hi_all, vols)):
                idx_lo = max(0, idx_lo)
    ...
    profile_cols = ['f__profile__poc', 'f__profile__vah', 'f__profile__val',
                   'f__profile__value_acceptance', 'f__profile__above_value', 'f__profile__below_value']
...
def compute_ict_structures(
    ...
    def compute_group_ict(group):
        ...
        bullish_fvg = (group['low'] > group['high'].shift(2)) & (group['close'].shift(1) > group['close'].shift(2))
        bearish_fvg = (group['high'] < group['low'].shift(2)) & (group['close'].shift(1) < group['close'].shift(2))
        ...
        disp_mask = (range_atr_ratio >= disp_atr_threshold) & (volume_filter >= disp_volume_threshold)
        ...
        in_discount = (group['close'] >= group['f__ict__pd_discount_bottom']) & (group['close'] <= group['f__ict__pd_discount_top'])
        in_premium = (group['close'] >= group['f__ict__pd_premium_bottom']) & (group['close'] <= group['f__ict__pd_premium_top'])
        ...
        sweep_high = (group['high'] > rolling_high.max().shift(1)) & (group['close'] < rolling_high.max().shift(1))
        sweep_low = (group['low'] < rolling_low.min().shift(1)) & (group['close'] > rolling_low.min().shift(1))
        ...
        group['f__ict__liq_sweep_high_level'] = np.where(
            group['f__ict__liq_sweep_high'],
            rolling_high.max().shift(1),
            np.nan
        )
        group['f__ict__liq_sweep_low_level'] = np.where(
            group['f__ict__liq_sweep_low'],
            rolling_low.min().shift(1),
            np.nan
        )
    ...
    ict_cols = [
        'f__ict__fvg_bull_lower', 'f__ict__fvg_bull_upper', 'f__ict__fvg_bull_active',
        'f__ict__fvg_bear_lower', 'f__ict__fvg_bear_upper', 'f__ict__fvg_bear_active',
        'f__ict__disp_high', 'f__ict__disp_low',
        'f__ict__pd_discount_top', 'f__ict__pd_discount_bottom',
        'f__ict__pd_premium_bottom', 'f__ict__pd_premium_top',
        'f__ict__in_discount', 'f__ict__in_premium',
        'f__ict__liq_sweep_high', 'f__ict__liq_sweep_low',
        'f__ict__liq_sweep_high_level', 'f__ict__liq_sweep_low_level'
    ]
...
def compute_order_flow_vpa(
    ...
    print(f"[DEBUG] compute_order_flow_vpa START: {len(result)} rows, {result['symbol'].nunique()} symbols", file=sys.stderr, flush=True)
    ...
    def compute_group_flow_vpa(group):
        ...
        print(f"  [DEBUG] group BEFORE rank(pct=True): elapsed {time.time() - group_start:.2f}s", file=sys.stderr, flush=True)
        with warnings.catch_warnings():
            warnings.filterwarnings('ignore', 'Mean of empty slice', RuntimeWarning)
            if len(group) >= 20:
                print(f"  [DEBUG] Calling rank(pct=True) on {len(group)} rows", file=sys.stderr, flush=True)
                volume_pct = group['volume'].rolling(window=50, min_periods=20).rank(pct=True)
                print(f"  [DEBUG] rank(pct=True) COMPLETE: elapsed {time.time() - group_start:.2f}s", file=sys.stderr, flush=True)
            else:
                volume_pct = pd.Series(np.nan, index=group.index)
        
        wick_ratio = ((group['high'] - group[['open', 'close']].max(axis=1)) +
                     (group[['open', 'close']].min(axis=1) - group['low'])) / tr.replace(0, tick)
    ...
    print(f"[DEBUG] groupby.apply COMPLETE: elapsed {time.time() - step_start:.2f}s", file=sys.stderr, flush=True)

    # Extract order flow and VPA features and merge back
    flow_vpa_cols = [
        'f__flow__ofi', 'f__flow__ofi_trend',
        'f__vpa__absorption', 'f__vpa__climax',
        'f__vpa__upthrust', 'f__vpa__downthrust',
    ...
    print(f"[DEBUG] compute_order_flow_vpa COMPLETE: total elapsed {time.time() - step_start:.2f}s", file=sys.stderr, flush=True)
...
def compute_all_regime_enhanced_features(
    ...
    *,
    ...
    regime_features = [col for col in result.columns if col.startswith(('f__anchor__', 'f__profile__', 'f__ict__', 'f__flow__', 'f__vpa__', 'f__stress__'))]
