...
def check_gold_consistency():
    ...
    for ticker_dir in gold_root.iterdir():
        ...
        if parquet_files:
            structure_issues.append(f"{ticker}: Has flat structure (parquet files in root)")
        ...
        for year_dir in year_dirs[:2]:  # Check max 2 years per ticker
            ...
            try:
                ...
                if 'ts' in df.columns:
                    ...
                    if has_tz:
                        timezone_issues.append(f"{ticker}/{year_dir.name}: Has timezone info ({df['ts'].dt.tz})")
                    
                    # Check sample timestamp format
                    sample_ts = df['ts'].iloc[0] if len(df) > 0 else None
                    logger.debug(f"{ticker}/{year_dir.name}: {ts_dtype}, tz={has_tz}, sample={sample_ts}")
    ...
    for ticker in sample_tickers:
        ...
        # Check structure
        year_dirs = sorted([d.name for d in ticker_path.iterdir() if d.is_dir() and d.name.isdigit()])
