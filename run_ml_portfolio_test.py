...
try:
    ...
    from qx_report.summaries import PerRunSummaries, TradeAnalysis
...
def load_strategy_bars(
    root: str, symbols: list[str], dates: list[str], aggregate_to_5m: bool = False
):
            ...
            if os.path.exists(parquet_path):
                try:
                    ...
                    if not df.empty:
                        ...
                        dfs.append(df)
                        print(
                            f"✅ Loaded {len(df)} bars for {symbol} {month_key} ({'5m' if aggregate_to_5m else '1m'})"
                        )
    ...
    for symbol in symbols:
        ...
        if "f__ta__vwap_30" in symbol_data.columns:
            ...
            symbol_data["vwap_deviation"] = (
                symbol_data[close_col] - symbol_data[vwap_col]
            ) / symbol_data[vwap_col]
            ...
            exit_threshold = (
                std_dev * 0.1
            )  # EXIT when very close to VWAP (tighter to allow same-day exit to trigger)
            ...
            signal_times = []
            min_signal_spacing = (
                config.get("min_signal_spacing_minutes", 5) * 60 * 1e9
            )  # 5 minutes for 1m intraday
            ...
            for _, row in symbol_data.iterrows():
                ...
                market_close_time = current_datetime.replace(
                ...
                if current_position is not None:
                    ...
                    if entry_date:
                        ...
                        if days_diff > 0:
                            print(
                                f"   📅 DATE CHECK: {symbol} Entry={entry_date}, Current={current_date}, Days={days_diff}"
                            )
                    ...
                    if not should_exit:
                        if current_position["type"] == "long":
                            ...
                            if (
                                current_dev > exit_threshold
                            ):  # Back to neutral (more forgiving exit)
                                should_exit = True
                                exit_reason = "mean_reversion_exit"
                            elif current_price < current_position["entry_price"] * (
                                1 - current_position["stop_loss_pct"]
                            ):
                                should_exit = True
                                exit_reason = "intraday_stop_loss"
                            elif current_price > current_position["entry_price"] * (
                                1 + current_position["take_profit_pct"]
                            ):
                                should_exit = True
                ...
                if current_position is None:
                    ...
                    entry_cutoff_time = current_datetime.replace(
            ...
            print(
                f"   📊 {symbol.upper()}: VWAP deviation [{min_dev:.4f}, {max_dev:.4f}], std={std_dev:.4f}"
            )
            print(
                f"   🎯 {symbol.upper()}: LONG={long_signals}, SHORT={short_signals}, EXIT={exit_signals} [INTRADAY]"
            )
        ...
        if "trades" in engine_result:
            ...
            if hasattr(trades_df, "to_csv") and not trades_df.empty:
                ...
                if not paired_trades.empty:
                    ...
                    print(
                        f"✅ Saved {len(paired_trades)} paired trades to {run_dir / 'trades.csv'}"
                    )
...
def run_ml_portfolio_test():
    ...
    try:
        ...
        train_signals = generate_trading_signals(
            train_features,
            ["aapl", "msft"],
            {"buy_deviation_threshold": -0.015, "sell_deviation_threshold": 0.015},
        )
        ...
        train_orders = intraday_ml_size_orders(
            signals=train_signals, bars=train_bars, config=train_risk_config
        )
        ...
        test_orders = intraday_ml_size_orders(
            signals=test_signals, bars=test_bars, config=train_risk_config
        )
        ...
        backtest_hash = intraday_ml_get_backtest_hash(
            bars=test_bars, orders=test_orders, cfg=backtest_config
        )
        ...
        run_dir = save_run_artifacts(
            run_id, backtest_results, test_bars, test_orders, backtest_config
        )
