...
def compute_comprehensive_features(bars: pd.DataFrame) -> pd.DataFrame:
    ...
    for lb in [5, 10, 20, 50]:
        ...
        df[f"price_vs_sma_{lb}"] = (
            (df["close"] - df[f"sma_{lb}"]) / df[f"sma_{lb}"] * 10000
        )  # bps
        df[f"price_vs_ema_{lb}"] = (
            (df["close"] - df[f"ema_{lb}"]) / df[f"ema_{lb}"] * 10000
        )

    # MA alignment
    df["ma_align_short"] = (
        np.sign(df["price_vs_sma_5"])
        + np.sign(df["price_vs_sma_10"])
        + np.sign(df["price_vs_sma_20"])
    ) / 3
    ...
    for lb in [10, 20]:
        ...
        df[f"bb_position_{lb}"] = (df["close"] - df[f"bb_lower_{lb}"]) / (
            4 * std + 1e-8
        )
        df[f"bb_squeeze_{lb}"] = (
            df[f"bb_width_{lb}"] < df[f"bb_width_{lb}"].rolling(20).mean() * 0.8
        ).astype(int)
    ...
    df["vol_price_confirm"] = ((df["returns"] > 0) & (df["rel_vol"] > 1)) | (
        (df["returns"] < 0) & (df["rel_vol"] > 1)
    )

    # Climax detection
    df["buying_climax"] = (
        (df["returns"] > df["returns"].rolling(20).quantile(0.9)) & (df["rel_vol"] > 2)
    ).astype(int)
    df["selling_climax"] = (
        (df["returns"] < df["returns"].rolling(20).quantile(0.1)) & (df["rel_vol"] > 2)
    ).astype(int)
    ...
    df["dist_to_swing_high"] = (
        (df["high"].rolling(20).max() - df["close"]) / df["close"] * 10000
    )
    df["dist_to_swing_low"] = (
        (df["close"] - df["low"].rolling(20).min()) / df["close"] * 10000
    )
    ...
    df["bull_ob"] = (
        large_candle & (df["body"] > 0) & (df["returns"].shift(-1) < 0)
    ).astype(int)
    df["bear_ob"] = (
        large_candle & (df["body"] < 0) & (df["returns"].shift(-1) > 0)
    ).astype(int)
    ...
    df["displacement_up"] = (
        df["returns"] > df["returns"].rolling(20).std() * 2
    ).astype(int)
    ...
    for lb in [10, 20, 50]:
        ...
        df[f"range_position_{lb}"] = (df["close"] - df[f"support_{lb}"]) / (
            df[f"resistance_{lb}"] - df[f"support_{lb}"] + 1e-8
        )
        df[f"near_resistance_{lb}"] = (
            df["close"] > df[f"resistance_{lb}"] * 0.995
        ).astype(int)
        df[f"near_support_{lb}"] = (df["close"] < df[f"support_{lb}"] * 1.005).astype(
            int
        )
    ...
    df["hammer"] = (
        (df["body"] > 0) & ((df["open"] - df["low"]) > 2 * abs(df["body"]))
    ).astype(int)
    df["shooting_star"] = (
        (df["body"] < 0) & ((df["high"] - df["open"]) > 2 * abs(df["body"]))
    ).astype(int)
    df["engulfing_bull"] = (
        (df["body"] > 0)
        & (df["body"].shift(1) < 0)
    ...
    df["trending"] = (
        df["adx_proxy"] > df["adx_proxy"].rolling(20).quantile(0.7)
    ).astype(int)
    df["ranging"] = (
        df["adx_proxy"] < df["adx_proxy"].rolling(20).quantile(0.3)
    ).astype(int)
...
def run_correlation_analysis():
    ...
    for symbol in symbols:
        ...
        for h in [60, 300]:
            ...
            l2_sym[f"fwd_ret_{h}s"] = (
                np.array(fwd_prices) / l2_sym["mid"].values - 1
            ) * 10000

        logger.info(
            f"{symbol}: {len(l2_sym):,} L2 records, {l2_sym['fwd_ret_300s'].notna().sum():,} valid fwd returns"
        )
    ...
    for _, row in corr_df.head(30).iterrows():
        ...
        logger.info(
            f"{row['type']:8} {row['feature']:30} {row['correlation']:+.4f} {sig}"
        )
    ...
    for cat_name, keywords in categories.items():
        cat_feats = corr_df[
            corr_df["feature"].apply(lambda x: any(k in x for k in keywords))
        ]
        if len(cat_feats) > 0:
    ...
    conditions = {
        ...
        "With momentum (mom_15 aligned)": (
            (signals["l2_signal"] == 1) & (signals["mom_15"] > 0)
        )
        | ((signals["l2_signal"] == -1) & (signals["mom_15"] < 0)),
        "Against momentum": ((signals["l2_signal"] == 1) & (signals["mom_15"] < 0))
        | ((signals["l2_signal"] == -1) & (signals["mom_15"] > 0)),
    }
    ...
    regime_df = pd.DataFrame(regime_results).sort_values(
        "improvement_bps", ascending=False
    )
    ...
    for _, row in regime_df.iterrows():
        ...
        logger.info(
            f"{row['condition']:35} {row['mean_ret_bps']:+.2f} bps ({sign}{row['improvement_bps']:.2f}), {row['win_rate']:.1f}% WR, n={row['n_signals']}"
        )
