...
# Polygon API (use existing key from environment or config)
import os
POLYGON_API_KEY = os.getenv("POLYGON_API_KEY")


def load_l2_data() -> pd.DataFrame:
...
def merge_l2_with_context(l2_df: pd.DataFrame, bars_dict: dict) -> pd.DataFrame:
    ...
    for symbol in l2_df["symbol"].unique():
        ...
        merged = pd.merge_asof(
            ...
            bars[["minute", "vwap_dist", "rsi_14", "mom_5", "mom_15", "atr_pct", "rel_vol"]].sort_values("minute"),
            on="minute",
            direction="backward",
        )
...
def compute_forward_returns(df: pd.DataFrame, horizons: list[int] = [5, 10, 15, 30]) -> pd.DataFrame:
...
def generate_signals(df: pd.DataFrame) -> pd.DataFrame:
    ...
    result.loc[result["obi_1"] > 0.3, "signal_l2_only"] = 1  # Buy
    result.loc[result["obi_1"] < -0.3, "signal_l2_only"] = -1  # Sell
    ...
    result.loc[no_context, "signal_l2_context"] = result.loc[no_context, "signal_l2_only"]
...
def analyze_signal_performance(df: pd.DataFrame) -> dict:
    ...
    for signal_col in ["signal_l2_only", "signal_l2_context"]:
        ...
        for h in [5, 10, 15, 30]:
            ...
            metrics[f"sharpe_{h}s"] = valid.mean() / valid.std() * np.sqrt(len(valid)) if valid.std() > 0 else 0
...
def run_analysis():
    ...
    for signal_name, metrics in results.items():
        ...
        logger.info(f"  Buy/Sell: {metrics['buy_signals']:,} / {metrics['sell_signals']:,}")
    ...
    # 9. Save results
    output_file = OUTPUT_DIR / f"analysis_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    ...
    if "l2_only" in results and "l2_context" in results:
        ...
        for h in [10, 15]:
            if f"mean_ret_{h}s_bps" in l2_only and f"mean_ret_{h}s_bps" in l2_ctx:
                improvement = l2_ctx[f"mean_ret_{h}s_bps"] - l2_only[f"mean_ret_{h}s_bps"]
                logger.info(f"\n{h}s Horizon:")
                logger.info(f"  L2-only mean return: {l2_only[f'mean_ret_{h}s_bps']:.2f} bps")
                logger.info(f"  L2+Context mean return: {l2_ctx[f'mean_ret_{h}s_bps']:.2f} bps")
