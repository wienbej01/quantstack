...
class FillProcessor:
    ...
    def process_fill(
        ...
        if tracked_order.intent == "ENTRY":
            ...
            logger.info(
                f"Fill for {tracked_order.intent} order {order_id}: {qty}@{price}"
            )

    def _process_entry_fill(
        self, order: TrackedOrder, fill_qty: int, fill_price: float, is_partial: bool
    ) -> None:
    ...
    def _process_exit_fill(
        ...
        position.exit_fills.append(
            {"qty": fill_qty, "price": fill_price, "reason": order.intent}
        )
