    ...
    def wait_for_market_open(self) -> None:
        ...
        while not self.is_trading_time():
            ...
            if not self.is_market_day():
                ...
                logger.info(f"Market closed. Waiting {wait_seconds/3600:.1f} hours until next check")
                time.sleep(min(3600, wait_seconds))  # Check every hour max
            else:
                # Market day but not trading time yet
                logger.info(f"Waiting for trading time. Current ET: {et_now.strftime('%H:%M:%S')}")
