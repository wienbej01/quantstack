...
def test_l2_snapshot_format():
    ...
    try:
        ...
        if missing:
            ...
            logger.error("  Signal generator cannot process snapshots without these fields!")
            return False
        else:
...
def test_signal_generation():
    ...
    try:
        ...
        logger.info(f"✓ Signal generated: {signal.signal_type.name} (strength={signal.strength:.2f}, confidence={signal.confidence:.2f})")
...
def main():
    ...
    if all_passed:
        ...
        logger.error("CRITICAL ISSUE: L2DataFeed does not provide required fields for signal generation!")
