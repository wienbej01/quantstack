...
def run(policy_config_path: str, backtest_config: dict) -> None:
    ...
    signals = decision_policy(features, probabilities, **policy_config)
    ...
    for timestamp, signal_row in signals.iterrows():
        if signal_row["signal"] != 0:
            ...
            eod_time = pd.to_datetime(
                timestamp.date().strftime("%Y-%m-%d")
                + " "
                + backtest_config["timing"]["eod_liquidation_time"]
            )
            if (
                entry_timestamp >= eod_time
                or entry_timestamp.time()
                >= pd.to_datetime(policy_config["block_new_entries_after_et"]).time()
            ):
                late_dropped += 1
                continue
