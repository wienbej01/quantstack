...
def run_metrics_consistency_check(backtest_config: dict) -> None:
    ...
    minute_sharpe = np.sqrt(backtest_config['annualize']['trading_days_per_year'] * backtest_config['annualize']['minute_bars_per_day']) * minute_returns.mean() / minute_returns.std()

    daily_returns = equity_df['equity'].resample('D').last().pct_change().dropna()
    daily_sharpe = np.sqrt(backtest_config['annualize']['trading_days_per_year']) * daily_returns.mean() / daily_returns.std()
