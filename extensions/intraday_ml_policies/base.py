...
class BaseMLPolicy(ABC):
    ...
    def decide(
        ...
        try:
            ...
            prediction_results = self.predictor.predict(self.model_id, pd.DataFrame([features]), return_probabilities=True)
            ...
            confidence = prediction_result.prediction_probability[0] if prediction_result.prediction_probability else 0.5
    ...
    def get_metrics(self) -> PolicyMetrics:
        ...
        if self.last_decision_time and self.metrics.total_decisions > 1:
            ...
            if time_diff > 0:
                self.metrics.decision_frequency = self.metrics.total_decisions / (time_diff / 3600.0)
    ...
    def _adjust_confidence(
        ...
        current_position = self.position_state.get(symbol, 0.0)
        position_ratio = abs(current_position) / self.max_position_size if self.max_position_size > 0 else 0
    ...
    def _create_decision(
        ...
        elif signal in [PolicySignal.STRONG_SELL, PolicySignal.SELL, PolicySignal.WEAK_SELL]:
