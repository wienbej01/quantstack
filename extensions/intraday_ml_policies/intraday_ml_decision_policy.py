...
from extensions.intraday_ml.policy.mediummove_policy_adapter import MediumMovePolicyAdapter
...
class EntryCandidate:
    ...
    prob_mediummove: float | None = None
...
class IntradayMLDecisionPolicy:
    ...
    def __init__(self, config: dict):
        ...
        self.mediummove_adapter: MediumMovePolicyAdapter | None = None
        if self.policy_mode == "bigmove":
            self.bigmove_adapter = BigMovePolicyAdapter(config.get("bigmove_policy"))
        elif self.policy_mode == "mediummove":
            self.mediummove_adapter = MediumMovePolicyAdapter(config.get("mediummove_policy"))
    ...
    def process_signals(self, signals: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
        ...
        for _, group in grouped:
            ...
            for _, row in group.iterrows():
                ...
                if self.bigmove_adapter is not None or self.mediummove_adapter is not None:
                    # In big/medium-move modes the adapter already encodes conviction into prob_long/prob_short.
                    # Neutral dominance would otherwise block almost all entries, so ignore neutral mass.
                    prob_neutral = 0.0
    ...
    def _evaluate_entry_candidate(
        ...
        *,
        ...
        mediummove_prob = self._clean_float(row.get("prob_mediummove"))
        ...
        if self.mediummove_adapter is not None:
            mediummove_allowed = bool(row.get("_mediummove_allowed", False))
            if not mediummove_allowed:
                self._append_rejection_record(
                    rejections,
                    dt_utc,
                    symbol,
                    "mediummove_prob_below_threshold",
                    prob_long=prob_long,
                    prob_short=prob_short,
                    directional_gap=directional_gap,
                    expected_r=self._clean_float(row.get("_mediummove_expected_r")),
                    prob_mediummove=mediummove_prob,
                    reason_key=REJECT_REASON_BIGMOVE_PROB,
                )
                return None
            if mediummove_prob is None:
                self._append_rejection_record(
                    rejections,
                    dt_utc,
                    symbol,
                    "mediummove_signal_missing",
                    prob_long=prob_long,
                    prob_short=prob_short,
                    directional_gap=directional_gap,
                    reason_key=REJECT_REASON_BIGMOVE_PROB,
        ...
        if self.mediummove_adapter is not None:
            mediummove_expected_r = self._clean_float(row.get("_mediummove_expected_r"))
            if mediummove_expected_r is not None:
                base_value = expected_r_value if expected_r_value is not None else 0.0
                expected_r_value = max(base_value, mediummove_expected_r)
                risk_metadata["mediummove_expected_r"] = mediummove_expected_r
        if (
    ...
    def _prepare_signals(self, signals: pd.DataFrame) -> pd.DataFrame:
        ...
        elif self.mediummove_adapter is not None:
            prepared = self.mediummove_adapter.transform(prepared)
