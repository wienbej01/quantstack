...
class MLStrategyOHLC(bt.Strategy):
    """Strategy with OHLC stop/target checking for coarse bars."""
    ...
    def __init__(self):
        self.orders_by_ts = {}
        self.active_positions = {}  # Track positions with stop/target
        self.trade_log = []
    ...
    def next(self):
        """Execute on each bar with OHLC stop/target checking."""
        current_dt = pd.Timestamp(self.datas[0].datetime.datetime(0))
        if current_dt.tzinfo is None:
            current_dt = current_dt.tz_localize('UTC')
        
        # Check EOD close (15:55 ET)
        current_et = current_dt.tz_convert('America/New_York')
        eod_time = pd.Timestamp(self.params.eod_close_time).time()
        
        if current_et.time() >= eod_time:
        ...
        # Check existing positions for stop/target hits using OHLC
        for symbol in list(self.active_positions.keys()):
            ...
            bar_high = data.high[0]
            bar_low = data.low[0]
            bar_close = data.close[0]
            ...
            if pos_info['side'] == 'LONG':
                ...
                if bar_low <= pos_info['stop_price']:
                    should_exit = True
                    exit_price = pos_info['stop_price']
                    exit_reason = 'STOP'
                # Check if high breached target
                elif bar_high >= pos_info['target_price']:
                    should_exit = True
            ...
            if should_exit:
                self.close(data)
                pos_info['exit_reason'] = exit_reason
                pos_info['exit_price_intended'] = exit_price
    ...
    def notify_trade(self, trade):
        ...
        if trade.isclosed:
            ...
            # Use exit reason from OHLC check
...
def run_backtest_ohlc(
    ...
    """Run backtest with OHLC stop/target monitoring."""
    ...
    cerebro.addstrategy(
        ...
        eod_close_time=cfg.get('eod_close_time', '15:55'),
    )
