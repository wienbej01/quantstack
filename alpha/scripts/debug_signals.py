...
print(
    f"  Duration: {(l2_df['ts_utc'].max() - l2_df['ts_utc'].min()).total_seconds() / 60:.1f} minutes"
)

# Check for order book imbalance
if "bid_px_1" in l2_df.columns and "ask_px_1" in l2_df.columns:
    l2_df["bid_size_total"] = l2_df[
        [f"bid_sz_{i}" for i in range(1, 11) if f"bid_sz_{i}" in l2_df.columns]
    ].sum(axis=1)
    l2_df["ask_size_total"] = l2_df[
        [f"ask_sz_{i}" for i in range(1, 11) if f"ask_sz_{i}" in l2_df.columns]
    ].sum(axis=1)
    l2_df["book_imbalance"] = (l2_df["bid_size_total"] - l2_df["ask_size_total"]) / (
        l2_df["bid_size_total"] + l2_df["ask_size_total"]
    )
    ...
    print(
        f"  |Imbalance| > 0.20: {(l2_df['book_imbalance'].abs() > 0.20).sum()} snapshots ({(l2_df['book_imbalance'].abs() > 0.20).sum() / len(l2_df) * 100:.1f}%)"
    )
    print(
        f"  |Imbalance| > 0.35: {(l2_df['book_imbalance'].abs() > 0.35).sum()} snapshots ({(l2_df['book_imbalance'].abs() > 0.35).sum() / len(l2_df) * 100:.1f}%)"
    )
...
if "l1_bid" in l2_df.columns and "l1_ask" in l2_df.columns:
    ...
    print(
        f"  Spread < 0.05%: {(l2_df['spread_pct'] < 0.05).sum()} snapshots ({(l2_df['spread_pct'] < 0.05).sum() / len(l2_df) * 100:.1f}%)"
    )
    print(
        f"  Spread < 0.10%: {(l2_df['spread_pct'] < 0.10).sum()} snapshots ({(l2_df['spread_pct'] < 0.10).sum() / len(l2_df) * 100:.1f}%)"
    )
