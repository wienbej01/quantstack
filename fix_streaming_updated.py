...
def convert_to_gold(bronze_df):
    ...
    gold_df["ts"] = (
        pd.to_datetime(bronze_df["ts_ms"], unit="ms", utc=True)
    ...
    gold_df["log_ret_1m"] = np.log(gold_df["close"] / gold_df["close"].shift(1)).fillna(
        0.0
    )

    # Session-based calculations
    gold_df["first_open"] = session_groups["open"].transform("first")
    gold_df["ret_from_open"] = (gold_df["close"] - gold_df["first_open"]) / gold_df[
        "first_open"
    ]
    ...
    if "vwap" in bronze_df.columns:
        ...
        gold_df["vwap_session"] = (
            gold_df["cum_dollar_vol"] / gold_df["cum_volume"]
        ).fillna(0)
    else:
    ...
    gold_df["prev_session_close"] = (
        gold_df["session_date"].map(prev_closes).astype("float64")
    )
...
def main():
    ...
    try:
        tickers = [
            ...
            if os.path.isdir(os.path.join(bronze_root, d)) and d.isupper()
        ]
    ...
    for i, ticker in enumerate(tickers):
        ...
        try:
            years = [
                ...
                if os.path.isdir(os.path.join(ticker_path, d))
            ]
        except:
            continue
    ...
    logger.info(
        f"Complete: {migrated} migrated, {skipped} skipped, {failed} failed in {elapsed/60:.1f}min"
    )
