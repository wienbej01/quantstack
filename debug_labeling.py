...
def debug_labeling():
    ...
    df = load_bars(root="/home/jacobw/gcs-mount", family="bars_1m", symbols=SYMBOLS, dates=[DATE, "2023-01-04"])
    if df.empty:
    ...
    atr = tr.groupby(df_out['symbol']).rolling(labeler.atr_window, min_periods=1).mean().reset_index(level=0, drop=True)
    
    df_out['atr'] = atr
    df_out['threshold'] = df_out['atr'] * labeler.atr_multiplier

    future_highs_grouped = df_out.groupby('symbol')['high'].rolling(window=labeler.horizon, min_periods=1).max()
    future_highs = future_highs_grouped.groupby(level='symbol').shift(-labeler.horizon).reset_index(level=0, drop=True)
    future_lows_grouped = df_out.groupby('symbol')['low'].rolling(window=labeler.horizon, min_periods=1).min()
    future_lows = future_lows_grouped.groupby(level='symbol').shift(-labeler.horizon).reset_index(level=0, drop=True)
    ...
    debug_cols = ['ts', 'close', 'atr', 'threshold', 'upper_barrier', 'lower_barrier', 'future_high', 'future_low']
