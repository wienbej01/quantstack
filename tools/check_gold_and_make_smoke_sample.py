...
def main():
    ...
    ap.add_argument(
        "--write-sample",
        action="store_true",
        help="Write a tiny sample parquet for smoke test",
    )
    ap.add_argument(
        "--out-dir",
        default="/tmp/e2e_smoke_from_gold",
        help="Where to write the sample if requested",
    )
    args = ap.parse_args()
    ...
    if args.family == "bars_1m":
        ...
        month_glob = os.path.join(
            ...
            args.symbol,
            args.year,
            f"{args.year}-{args.month}.parquet",
        )
    else:
