...
class PositionTracker:
    ...
    def update_from_fill(
        self, symbol: str, system: str, side: str, qty: int, price: float
    ):
    ...
    def reconcile_with_ibkr(self) -> list[dict]:
        ...
        try:
            cur = conn.cursor()
            cur.execute(
                "SELECT symbol, system, quantity FROM positions WHERE quantity != 0"
            )
            db_positions = {}
            ...
            for symbol in all_symbols:
                ...
                if ibkr_qty != db_qty:
                    ...
                    logger.warning(
                        f"Position mismatch {symbol}: IBKR={ibkr_qty} DB={db_qty}"
                    )
