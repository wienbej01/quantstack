...
with open("/home/jacobw/api-exported-logs.txt", "r") as f:
    for line in f:
        if "<- [3;" in line and "IOC" in line:
            ...
            match = re.search(
                r"(\d{2}:\d{2}:\d{2}):\d{3}.*<- \[3;(\d+);.*?;([A-Z]+);.*?;(BUY|SELL);.*?;LMT;([\d.]+)",
                line,
            )
            if match:
...
for o in orders[:20]:
    ...
    if abs(o["price"] - rounded) > 0.0001:
        print(
            f"  ❌ Order {o['id']}: {o['action']} {o['symbol']} @ ${o['price']:.6f} (should be ${rounded:.2f})"
        )
        invalid_count += 1
    else:
...
for oid in errors[:10]:
    if oid in bracket_orders:
        ...
        rounded = round_to_tick(o["price"])
        print(
            f"  Order {oid}: {o['action']} {o['symbol']} {o['type']} @ ${o['price']:.6f} → should be ${rounded:.2f}"
        )
