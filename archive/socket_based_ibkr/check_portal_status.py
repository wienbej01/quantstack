    ...
    try:
        ...
        if positions:
            for pos in positions:
                ...
                print(
                    f"{pos.contract.symbol}: {pos.position} shares @ ${pos.avgCost:.2f} | P&L: ${pnl:.2f}"
                )
        else:
        ...
        if recent_execs:
            ...
            for ex in recent_execs[-20:]:  # Last 20
                ...
                print(
                    f"{time_str} | {ex.contract.symbol:>6} | {ex.side:>4} | {ex.shares:>3} | ${ex.price:>7.2f} | {ex.orderId}"
                )
        else:
        ...
        if open_orders:
            for order in open_orders:
                print(
                    f"Order {order.orderId}: {order.action} {order.totalQuantity} {order.contract.symbol} @ ${order.lmtPrice}"
                )
        else:
