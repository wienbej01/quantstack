...
def debug_trading_gates():
    ...
    print("\n4. Applying features...")
    try:
        features_config = [{"type": "core_basics", "params": {"vwap_window_m": 30}}]
        data_with_features = apply(data, features_config)
        print("✅ Features applied")
        print(
            f"   New columns: {[col for col in data_with_features.columns if col.startswith('f__')]}"
        )
    except Exception as e:
    ...
    if buy_signals == 0 and sell_signals == 0:
        ...
        print("   1. VWAP calculation window too long (30 minutes)")
        print("   2. Trading thresholds too strict (±0.5%)")
        print("   3. Market conditions too stable (low volatility)")
