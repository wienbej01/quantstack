...
def main():
    ...
    try:
        ...
        logging.info(
            f"  Loaded {len(existing):,} rows, {len(existing.columns)} columns"
        )
        ...
        existing_filtered = existing.filter(
            pl.col("timestamp").dt.date() < pl.date(2025, 9, 10)
        )
        ...
        logging.info(
            "STEP 5: Processing single symbol with full feature engineering..."
        )
            ...
            for col in existing_cols:
                if col in df.columns:
                    if df[col].dtype != existing[col].dtype:
                        logging.error(
                            f"  {col}: {df[col].dtype} vs {existing[col].dtype}"
                        )
            return
