...
def build_complete_features(df):
    ...
    for window in [5, 10, 20]:
        df = df.with_columns([
            ((pl.col("returns") - pl.col("returns").rolling_mean(window, min_periods=1)) / 
             pl.col("returns").rolling_std(window, min_periods=1)).alias(f"ret_zscore_{window}")
        ])
    ...
    for fast, slow in [(5, 10), (12, 26), (8, 21)]:
        df = df.with_columns([
            (pl.col("close").ewm_mean(span=fast) - pl.col("close").ewm_mean(span=slow)).alias(f"macd_{fast}_{slow}"),
            pl.col(f"macd_{fast}_{slow}").ewm_mean(span=9).alias(f"macd_signal_{fast}_{slow}"),
        ])
        df = df.with_columns([
            (pl.col(f"macd_{fast}_{slow}") - pl.col(f"macd_signal_{fast}_{slow}")).alias(f"macd_hist_{fast}_{slow}")
        ])
    ...
    for window in [5, 14]:
        df = df.with_columns([
            pl.col("high").rolling_max(window, min_periods=1).alias(f"high_max_{window}"),
            pl.col("low").rolling_min(window, min_periods=1).alias(f"low_min_{window}"),
        ])
        df = df.with_columns([
            ((pl.col("close") - pl.col(f"low_min_{window}")) / 
             (pl.col(f"high_max_{window}") - pl.col(f"low_min_{window}"))).alias(f"stoch_{window}")
        ])
    ...
    for window in [5, 10, 20]:
        df = df.with_columns([
            ((pl.col("close") - pl.col("close").shift(window)) / 
             pl.col("close").shift(window)).alias(f"roc_{window}")
        ])
    ...
    for window in [10, 20]:
        df = df.with_columns([
            ((pl.col(f"high_max_{window}") - pl.col("close")) / 
             (pl.col(f"high_max_{window}") - pl.col(f"low_min_{window}"))).alias(f"williams_r_{window}")
        ])
    
    # ICT concepts - displacement and break of structure
    for window in [3, 5]:
        df = df.with_columns([
            (pl.col("close") - pl.col("close").shift(window)).alias(f"displacement_{window}"),
            (pl.col("high") > pl.col("high").shift(1)).cast(pl.Int8).alias(f"bos_up_{window}"),
            (pl.col("low") < pl.col("low").shift(1)).cast(pl.Int8).alias(f"bos_down_{window}"),
        ])
    
    # VPA concepts
    df = df.with_columns([
        (pl.col("volume") * pl.col("range")).alias("volume_spread"),
        (pl.col("close") - (pl.col("high") + pl.col("low")) / 2).alias("close_position"),
    ])
    ...
    for window in [5, 10, 20]:
        df = df.with_columns([
            (pl.col("returns").rolling_sum(window, min_periods=1) / 
             pl.col("returns").rolling_std(window, min_periods=1)).alias(f"rel_strength_{window}")
        ])
    ...
    feature_cols.extend([f"macd_signal_{f}_{s}" for f,s in [(5,10),(12,26),(8,21)]])
    ...
    feature_cols.extend(["cross_rank_ret", "cross_rank_vol", "sector_momentum", "cross_dispersion", "market_breadth", "up_down_ratio"])
...
def build_features_for_month(year, month):
    ...
    if all_features:
        ...
        logging.info(f"✅ {year}-{month:02d}: {len(month_features):,} feature rows with {len(month_features.columns)} features")
...
def main():
    ...
    logging.info(f"Total new features: {len(new_features):,} rows with {len(new_features.columns)} columns")
