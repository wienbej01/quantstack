...
def load_historical_bars(
    symbol, target_date, data_root="/home/jacobw/gcs-mount/gold/stocks/1m"
):
...
def engineer_features(df, target_date):
    ...
    df = df.with_columns(
        [
            ...
            (
                pl.col("close").ewm_mean(span=5) - pl.col("close").ewm_mean(span=10)
            ).alias("macd_5_10"),
            (
                pl.col("close").ewm_mean(span=8) - pl.col("close").ewm_mean(span=21)
            ).alias("macd_8_21"),
        ]
    )
    ...
    for name in ["macd_12_26", "macd_5_10", "macd_8_21"]:
        df = df.with_columns(
            [
                pl.col(name)
                .ewm_mean(span=9)
                .alias(f"{name.replace('macd', 'macd_signal')}")
            ]
        )
        df = df.with_columns(
            [
                (pl.col(name) - pl.col(f"{name.replace('macd', 'macd_signal')}")).alias(
                    f"{name.replace('macd', 'macd_hist')}"
                )
            ]
        )
    ...
    feature_cols = (
        ...
        + [f"ret_{w}bar" for w in [1, 2, 3, 5, 10, 15, 20, 30]]
        + [f"vol_{w}bar" for w in [1, 2, 3, 5, 10, 15, 20, 30]]
        ...
        + [
            f"bb_{t}_{w}"
            for t in ["upper", "lower", "position", "width"]
            for w in [10, 20]
        ]
    )
...
def main():
    logging.info("=" * 80)
    logging.info(
        "BUILDING MISSING FEATURES: Sep-Dec 2025 (FIXED - with historical data)"
    )
    logging.info("=" * 80)
