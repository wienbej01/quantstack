...
def analyze_gold_structure():
    ...
    if r2k_file.exists():
        try:
            ...
            if 'Ticker' in r2k_df.columns:
                ...
                logger.info(f"Russell 2000 tickers missing from gold: {len(r2k_missing)}")
    ...
    for file_path in sample_files:
        try:
            ...
            if 'ts' in df.columns:
                ...
                logger.info(f"{file_path.parent.parent.name}/{file_path.parent.name}: tz={tz_info}, sample={sample_ts}")
    ...
    for ticker_dir in list(gold_root.iterdir())[:10]:  # Sample first 10
        ...
        try:
            ...
            year_dirs = sorted([d for d in ticker_dir.iterdir() if d.is_dir() and d.name.isdigit()])
            if not year_dirs:
                continue
    ...
    if price_violations:
        ...
        for violation in price_violations[:5]:
            logger.info(f"  {violation['ticker']}: ${violation['min_price']:.2f} - ${violation['max_price']:.2f}")
...
def check_download_config():
    ...
    if config_file.exists():
        try:
            ...
            logger.info(f"Prefilter fraction min: {config.get('prefilter_frac_min', 'NOT SET')}")
