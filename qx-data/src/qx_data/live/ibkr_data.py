...
class IBKRMarketDataManager:
    ...
    def subscribe_symbols(self, symbols: List[str]):
        ...
        for symbol in symbols:
            try:
                ...
                ticker = self.ib.reqMktData(contract, '', True, False)  # snapshot=True for paper
    ...
    def get_historical_bars(self, symbol: str, duration: str = "1 D", bar_size: str = "1 min") -> pd.DataFrame:
    ...
    def get_all_historical_bars(self, symbols: List[str], duration: str = "1 D") -> Dict[str, pd.DataFrame]:
    ...
    def compute_cross_sectional_features(self, historical_data: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        for symbol in symbols:
            ...
            feat = {
                ...
                'vol_ratio': df['volume'].iloc[-5:].mean() / df['volume'].iloc[-20:].mean() if len(df) >= 20 else 1.0,
