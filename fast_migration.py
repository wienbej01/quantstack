...
def convert_bronze_to_gold_fast(bronze_df):
    ...
    # Column mapping
    col_map = {'t': 'ts_ms', 'o': 'open', 'h': 'high', 'l': 'low', 'c': 'close', 'v': 'volume', 'vw': 'vwap', 'n': 'trades'}
    ...
    gold_df = pd.DataFrame({
        'ts': pd.to_datetime(bronze_df['ts_ms'], unit='ms', utc=True).dt.tz_convert('US/Eastern').dt.tz_localize(None),
...
def main():
    ...
    for i, bronze_path in enumerate(bronze_files):
        if i % 1000 == 0:
            ...
            logger.info(f"Progress: {i}/{total_files} ({i/total_files*100:.1f}%) - {rate:.1f} files/sec")
