...
def main():
    ...
    for regime in ["bull", "bear", "sideways"]:
        ...
        X_train = (
            train_df.loc[regime_mask, feature_cols]
            .fillna(0)
    ...
    val_returns = (
        val_df["forward_return"].fillna(0)
        if "forward_return" in val_df.columns
        else val_df["returns"].shift(-30).fillna(0)
    )
    ...
    logging.info("=" * 80)
    logging.info(
        f"Long trades: {long_trades:,}, PnL: {long_pnl:.2%}, Win rate: {long_win_rate:.1%}"
    )
    logging.info(
        f"Short trades: {short_trades:,}, PnL: {short_pnl:.2%}, Win rate: {short_win_rate:.1%}"
    )
