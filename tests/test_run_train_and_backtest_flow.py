...
def mock_dependencies():
    ...
    with (patch('run_train_and_backtest.load_bars') as mock_load_bars,
        ...
        mock_load_bars.return_value = pd.DataFrame({'ts': [1,2,3], 'symbol':['BAC','BAC','BAC']})
        mock_resample_data.return_value = pd.DataFrame({'ts': [1, 2, 3], 'symbol':['BAC','BAC','BAC']})

        # Mock dataset and feature creation
        mock_create_training_dataset.return_value = pd.DataFrame({
            'ts': [1, 2, 3], 'symbol': ['BAC', 'BAC', 'BAC'], 'f__dummy': [1.0, 2.0, 3.0], 'label': [0, 1, -1]
        })
        mock_create_feature_set.return_value = pd.DataFrame({
            'ts': [4, 5, 6], 'symbol': ['BAC', 'BAC', 'BAC'], 'f__dummy': [4.0, 5.0, 6.0]
        })
        ...
        mock_trainer_instance.train_model.return_value = MagicMock(model=MagicMock(), metrics={'accuracy': 0.75})
        ...
        mock_engine_instance.run.return_value = MagicMock(to_dict=lambda: {'performance': {'total_return': 0.0}})
