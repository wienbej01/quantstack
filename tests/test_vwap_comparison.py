...
def test_enhanced_vwap_comparison():
    ...
    if len(revert_orders) > 0:
        assert len(revert_with_atr) >= len(revert_orders) * 0.5, "Most revert orders should have ATR info"
    if len(momentum_orders) > 0:
        assert len(momentum_with_atr) >= len(momentum_orders) * 0.5, "Most momentum orders should have ATR info"
