...
class TestEndToEndWorkflow:
    ...
    def test_feature_selection_integration(self, sample_intraday_data):
        ...
        result_mutual = selector.select_mutual_info(
            X=features, y=target, k=3
        )

        # RFE selection
        result_rfe = selector.select_rfe(
            X=features, y=target, n_features=3
        )
    ...
    def test_monitoring_integration(self, sample_intraday_data):
        ...
        probabilities = np.random.beta(2, 2, len(sample_intraday_data))  # Realistic confidence distribution
...
class TestPerformanceRequirements:
    ...
    def test_intraday_compliance_no_future_leak(self, sample_intraday_data):
        ...
        for col in lagged_data.columns:
            if "_lag_1" in col:
                ...
                assert pd.isna(lagged_data[col].iloc[0]) or lagged_data[col].iloc[0] == sample_intraday_data[col.replace("_lag_1", "")].iloc[0]
    ...
    def test_reproducibility_with_seed(self, sample_intraday_data):
        ...
        selector = FeatureSelector()
        target = sample_intraday_data["close"].pct_change(3).shift(-3).dropna()
        features = sample_intraday_data.drop(columns=["close", "open", "high", "low", "volume"]).iloc[:-3]
