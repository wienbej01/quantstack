...
DAY_CHARACTERISTICS: dict[int, tuple[float, float]] = {
    0: (0.002, 0.008),  # Tuesday: uptrend
    1: (-0.001, 0.015),  # Wednesday: volatile, down trend
    2: (0.0, 0.006),  # Thursday: sideways
}
DEFAULT_DAY_CHARACTERISTIC = (0.001, 0.007)  # Friday: slight uptrend
SESSION_OPEN_SPIKE_MINUTES = 30
SESSION_CLOSE_START_MINUTE = 360
...
class TestCreateTrainingDataset:
    ...
    def bac_week_data(self):
        ...
        for day_idx, trading_day in enumerate(trading_days):
            ...
            for minute_idx, ts in enumerate(timestamps):
                ...
                if minute_idx < SESSION_OPEN_SPIKE_MINUTES:
                    volume_multiplier = 1.5
                    vol_multiplier = 1.2
                elif minute_idx > SESSION_CLOSE_START_MINUTE:
    ...
    def test_create_training_dataset_basic(
        ...
        mock_load_data.assert_called_once_with(
            symbols=["BAC"], start_date="2024-01-02", end_date="2024-01-05", config=None
        )
        ...
        assert "symbol" in result_df.columns or result_df.index.names == [
            "symbol",
            "ts",
    ...
    def test_manual_verification_no_lookahead(
        ...
        if "symbol" in result_df.columns:
            ...
            try:
                ...
                matching_rows = result_df[
                    result_df.index.get_level_values("ts") == middle_timestamp
                ]
        ...
        if not matching_rows.empty:
            ...
            assert label in [
                ...
                0,
                1,
            ], f"Invalid label {label} for timestamp {middle_timestamp}"
