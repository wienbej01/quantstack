...
def load_sp500_tickers():
    ...
    sp500_file = Path("/home/jacobw/quantstack/universe_data/sp500_tickers_retrieved.csv")
    ...
    try:
        ...
        if 'Ticker' in df.columns:
            ...
            for ticker in tickers:
                ...
                if ticker.replace('-', '').replace('.', '').isalpha() and len(ticker) <= 5:
...
def create_action_plan(coverage):
    ...
    logger.info(f"Download historical for existing: {len(actions['download_historical']):,}")
...
def main():
    ...
    logger.info(f"Can remove (2025-only): {summary['remove_2025_only_count']:,} directories")
    
    logger.info(f"\n=== NEXT STEPS ===")
    logger.info(f"1. Review files in: /home/jacobw/quantstack/universe_data/")
    logger.info(f"2. Download {summary['download_needed_count']:,} missing/incomplete tickers")
    logger.info(f"3. Remove {summary['remove_2025_only_count']:,} 2025-only directories")
