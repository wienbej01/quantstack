...
def remove_2025_only_data(tickers_to_remove, dry_run=True):
    ...
    for ticker in tickers_to_remove:
        ...
        if ticker_path.exists() and ticker_path.is_dir():
            ...
            if years == [2025]:
                ...
                try:
                    size = sum(
                        f.stat().st_size for f in ticker_path.rglob("*") if f.is_file()
                    )
                    space_freed += size
                    ...
                    if not dry_run:
                        ...
                        logger.debug(
                            f"Would remove: {ticker} ({size/1024/1024:.1f} MB)"
                        )
    ...
    logger.info(
        f"{'Would remove' if dry_run else 'Removed'} {removed_count} directories"
    )
...
def create_download_lists():
    ...
    complete_universe.to_csv(
        download_dir / "complete_universe_r2k_format.csv", index=False
    )
...
def update_data_download_config():
    ...
    backup_path = Path(
        "/home/jacobw/data_download/configs/config_backup_with_price_filter.yaml"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Clean up and prepare for complete universe downloads"
    )
    parser.add_argument(
        "--remove-2025-only",
        action="store_true",
