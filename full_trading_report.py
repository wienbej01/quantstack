...
def generate_full_trading_report(db_path: str = "/home/jacobw/intraday_stack/data/journal/events.db", 
    ...
    for col in ['entry_price', 'exit_price', 'gross_pnl', 'fees', 'net_pnl', 'entry_slippage', 'exit_slippage']:
        df[col] = df[col].round(4)
    
    # Add derived columns
    df['hold_time_min'] = (df['hold_time_seconds'] / 60).round(1)
    df['return_pct'] = ((df['exit_price'] - df['entry_price']) / df['entry_price'] * 100).round(2)
    df['return_pct'] = df.apply(lambda x: -x['return_pct'] if x['direction'] == 'short' else x['return_pct'], axis=1)
    
    # Reorder columns for better readability
    columns = [
        'trade_id', 'ticker', 'strategy', 'system', 'direction', 'status',
        'entry_time', 'entry_price', 'entry_qty',
        'exit_time', 'exit_price', 'exit_qty', 'exit_reason',
        'hold_time_min', 'return_pct',
...
def print_trading_summary(df: pd.DataFrame):
    ...
    if not closed_trades.empty:
        ...
        # By system
        print("\nBY SYSTEM:")
        system_summary = closed_trades.groupby('system').agg({
        ...
        # By strategy
        print("\nBY STRATEGY:")
        strategy_summary = closed_trades.groupby('strategy').agg({
            'net_pnl': ['count', 'sum', 'mean'],
            'fees': 'sum'
        }).round(2)
...
def main():
    ...
    parser.add_argument("--db", default="/home/jacobw/intraday_stack/data/journal/events.db", 
                       help="Database path")
