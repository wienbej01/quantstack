...
def check_bronze_consistency():
    ...
    for ticker_dir in bronze_root.iterdir():
        ...
        if sample_file:
            try:
                ...
                ts_cols = [col for col in df.columns if col in ['t', 'ts', 'ts_ms', 'timestamp']]
                if ts_cols:
                    ts_col = ts_cols[0]
                    ...
                    if 'datetime' in ts_dtype:
                        ...
                        stats['timezone_formats'][f"{ts_col}:{ts_dtype}:tz={tz_info}"] += 1
                    else:
    ...
    for cols, count in sorted(stats['column_formats'].items(), key=lambda x: x[1], reverse=True)[:5]:
        logger.info(f"  {count} files: {list(cols)}")
    
    logger.info(f"\nTimezone Formats:")
    for tz_format, count in sorted(stats['timezone_formats'].items(), key=lambda x: x[1], reverse=True):
