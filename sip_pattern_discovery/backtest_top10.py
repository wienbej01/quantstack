...
def load_monthly_cache_with_bins(
...
def backtest_top10(
    ...
    for pattern in TOP_10_PATTERNS:
        ...
        print(
            f"Horizon: {pattern['horizon']} ({pattern['horizon'].replace('fwd_ret_', '')})"
        )
    ...
    if not success.empty:
        ...
        for _, row in summary.iterrows():
            print(
                f"{row['pattern_id']:30s} | Exp: {row['expectancy']:.4%} | "
...
if __name__ == "__main__":
    ...
    output_path = (
        sys.argv[2] if len(sys.argv) > 2 else "output_aaa/backtest_top10_results.csv"
    )
