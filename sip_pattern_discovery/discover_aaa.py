...
def apply_aaa_filters(
    ...
    if require_event:
        patterns_df = patterns_df[patterns_df['rule'].apply(event_filter.is_event_based)]
    ...
    patterns_df['overfit_check'] = patterns_df.apply(
        lambda row: overfit_filter.is_overfit(row.to_dict()),
        axis=1
    )
    ...
    if require_regime_match and current_regime:
        ...
        print(f"  After regime filter: {len(patterns_df)}/{initial_count} patterns (regime: {current_regime})")
...
def main():
    ...
    if cache_file.exists() and spy_cache_file.exists():
        ...
        df, spy_df, metadata = load_sip_filtered_data(
            args.start_date,
            args.end_date,
            config['temporal_periods']['scan_months'] + config['temporal_periods']['validation_months'] + 1,
            sip_dir,
            gold_dir,
        )
    ...
    print(f"✅ Features and targets ready")
    ...
    if not args.skip_validation:
        ...
        print(f"  Scan period: {period_info['scan']['start']} to {period_info['scan']['end']} ({period_info['scan']['days']} days)")
        print(f"  Validation period: {period_info['validation']['start']} to {period_info['validation']['end']} ({period_info['validation']['days']} days)")
        print(f"  OOS period: {period_info['oos']['start']} to {period_info['oos']['end']} ({period_info['oos']['days']} days)")
    else:
        scan_df = df
    ...
    # Feature columns
    high_alpha_events = [
        "rel_underperform_extreme", "rel_outperform_extreme",
        "price_up_vol_weak", "price_down_vol_weak",
        "at_session_high", "at_session_low",
        "vwap_cross_up", "vwap_cross_down",
    ]
    state_features = [
        "ret_60m", "rel_strength_60m", "session_range_pct",
        "rvol", "atr_14", "price_vs_vwap_pct",
    ]
    time_context = ["is_first_hour", "is_power_hour"]

    feature_cols = [f for f in high_alpha_events + state_features + time_context if f in scan_df.columns]

    # Define regimes
    regimes = {
        "bull_low_vol": (scan_df["spy_above_sma20"] == True) & (scan_df.get("spy_high_vol", False) == False),
        "bull_high_vol": (scan_df["spy_above_sma20"] == True) & (scan_df.get("spy_high_vol", False) == True),
        "bear_low_vol": (scan_df["spy_above_sma20"] == False) & (scan_df.get("spy_high_vol", False) == False),
        "bear_high_vol": (scan_df["spy_above_sma20"] == False) & (scan_df.get("spy_high_vol", False) == True),
    }
    ...
    for horizon in horizons:
        ...
        for regime_name, regime_mask in regimes.items():
            ...
            print(f"\n{'=' * 80}")
            print(f"REGIME: {regime_name} | HORIZON: {horizon}m | SAMPLES: {len(df_regime):,}")
            print("=" * 80)
            ...
            long_patterns = discover_patterns(
                ...
                return_col,
                direction="LONG",
                min_t_stat=25.0,  # AAA minimum
                min_expectancy=0.02,  # AAA minimum
            ...
            if not long_patterns.empty:
                ...
                if not long_patterns.empty:
                    ...
                    print(f"✅ Saved {len(long_patterns)} AAA LONG patterns")
            ...
            if not short_patterns.empty:
                ...
                if not short_patterns.empty:
                    ...
                    print(f"✅ Saved {len(short_patterns)} AAA SHORT patterns")
    ...
    if all_patterns:
        ...
        if not args.skip_llm:
            ...
            llm_output = format_patterns_for_llm(top_patterns, "AAA Patterns", top_n=len(top_patterns))
