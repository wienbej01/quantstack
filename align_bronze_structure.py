...
STANDARD_COLUMNS = [
    'ts_ms', 'open', 'high', 'low', 'close', 'volume', 'vwap', 'trades',
    'ticker', 'vendor', 'ingested_at_ms', 'page', 'date_et', 'session'
]
    ...
    for ticker_dir in bronze_dir.iterdir():
        ...
        for file in parquet_files:
            ...
            try:
                ...
                new_path = year_dir / f"{year}-{file.stem.split('-')[-1] if '-' in file.stem else '01'}.parquet"
    ...
    for ticker_dir in bronze_dir.iterdir():
        ...
        for year_dir in ticker_dir.iterdir():
            ...
            for file_path in year_dir.glob("*.parquet"):
                ...
                if processed % 100 == 0:
                    logging.info(f"Processed {processed} files, standardized {standardized}")
