...
class GoldDataFixer:
    ...
    def check_price_filter(self, ticker_path, price_range=(5.0, 50.0), min_fraction=0.1):
        """Check if ticker meets price range criteria."""
        try:
            year_dirs = sorted([d for d in ticker_path.iterdir() if d.is_dir() and d.name.isdigit()])
            if not year_dirs:
                return False
            ...
            for file_path in sample_files[:10]:  # Limit to avoid excessive processing
                try:
                    ...
                    in_range = ((daily_closes >= price_range[0]) & (daily_closes <= price_range[1])).sum()
    ...
    def run_fix(self, dry_run=False):
        ...
        for i, ticker_dir in enumerate(ticker_dirs):
            if (i + 1) % 100 == 0:
                logger.info(f"Processed {i + 1}/{self.stats['total_tickers']} tickers...")
    ...
    def print_summary(self, dry_run=False):
        ...
        total_removed = (self.stats['removed_2025_only'] + 
...
def main():
    ...
    parser.add_argument('--gold-root', default='/home/jacobw/gcs-mount/gold/stocks/1m',
